    ...
    notebook["cells"].append(
        {
            ...
            "source": [
                ...
                "    items_with_product_desc = 0\n",
                "    items_with_aisle_format = 0\n",
                "    \n",
                "    for item in defragged_items:\n",
                "        if item.notes:\n",
                "            # Check for location format (e.g., \"B15:35-4\")\n",
                "            if any(char.isalpha() and any(c.isdigit() for c in item.notes) for char in item.notes[:3]):\n",
                "                items_with_location += 1\n",
                "            # Check for product description after pipe separator\n",
                "            if ' | ' in item.notes:\n",
                "                items_with_product_desc += 1\n",
                "            # Check for aisle format (letter + number: section-bay)\n",
                "            if ':' in item.notes and any(char.isalpha() for char in item.notes.split(':')[0]):\n",
                "                items_with_aisle_format += 1\n",
                "    \n",
                "    benefits_data = {\n",
                "        'Metric': [\n",
                "            'Total Items',\n",
                "            'Items with Location Info',\n",
                "            'Items with Product Description',\n",
                "            'Items with Aisle Format',\n",
                "            'Location Coverage',\n",
                "            'Organization Status'\n",
                "        ],\n",
                "        'Value': [\n",
                "            len(defragged_items),\n",
                "            items_with_location,\n",
                "            items_with_product_desc,\n",
                "            items_with_aisle_format,\n",
                "            f\"{(items_with_location/len(defragged_items)*100):.1f}%\",\n",
                "            \"✅ Organized by Aisle\"\n",
                "        ]\n",
                "    }\n",
                "    \n",
                ...
                "    \n",
                "    if items_with_location < len(defragged_items):\n",
                "        missing_count = len(defragged_items) - items_with_location\n",
                '        print(f"\\n💡 Note: {missing_count} item(s) don\'t have location information")\n',
                ...
                "        \n",
                "    if items_with_product_desc < len(defragged_items):\n",
                "        no_match_count = len(defragged_items) - items_with_product_desc\n",
