...
class TargetedSecurityTester:
        ...
        for endpoint in endpoints:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if sensitive_info:
                        logger.warning(
                            f"⚠️  SENSITIVE INFORMATION DISCLOSED: {endpoint}"
                        )
                        logger.warning(
                            f"   Sensitive fields: {list(sensitive_info.keys())}"
                        )
        ...
        for params in test_params:
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    logger.warning(
                        f"⚠️  POTENTIAL VULNERABILITY: Got data with params {params}"
                    )
                    logger.warning(
    ...
    def run_all_tests(self):
        ...
        logger.info(
            "✅ Targeted security testing completed. Results saved to targeted_security_results.json"
        )
        return results
