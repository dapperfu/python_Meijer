...
class MeijerStoreSearch:
    ...
    def find_stores_with_service(
        self, service_type: str, **search_kwargs
    ) -> List[MeijerStore]:
        """
...
if __name__ == "__main__":
    ...
    for store_id in test_store_ids:
        ...
        if store:
            print(
                f"   ✅ Found: {store.display_name} (ID: {store.store_id}, type: {type(store.store_id).__name__})"
            )
        else:
    ...
    stores = store_search.find_stores_nearby(
        latitude=42.82893, longitude=-86.09048, radius_miles=50, max_results=5
    )

    print(f"📍 Found {len(stores)} stores:")
    for store in stores:
        print(
            f"   - Store {store.store_id}: {store.display_name} ({store.distance_miles:.1f} mi)"
        )
