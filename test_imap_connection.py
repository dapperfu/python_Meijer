...
def test_imap_connection():
    ...
    try:
        ...
        try:
            ...
            if message_numbers[0]:
                ...
                for line in body_lines:
                    if any(keyword in line.lower() for keyword in verification_keywords):
                ...
                lines = email_body.split('\n')
                subject_line = next((line for line in lines if line.startswith('Subject:')), 'Subject: Not found')
                print(f"📝 {subject_line}")
                
                # Show first few content lines
                content_lines = [line for line in lines if line.strip() and not line.startswith(('From:', 'To:', 'Subject:', 'Date:', '-'))]
            ...
            if message_numbers[0]:
                ...
                if len(all_emails) > 0:
                    ...
                    lines = email_body.split('\n')
                    subject_line = next((line for line in lines if line.startswith('Subject:')), 'Subject: Not found')
                    from_line = next((line for line in lines if line.startswith('From:')), 'From: Not found')
                    print(f"📝 {subject_line}")
                    print(f"👤 {from_line}")
        ...
        try:
            _, verification_messages = mail.search(None, 'FROM "meijer" SUBJECT "verification"')
            
            if verification_messages[0]:
                verification_emails = verification_messages[0].split()
                print(f"✅ Found {len(verification_emails)} verification emails from Meijer")
            else:
