...
def extract_meijer_api_calls(flows: List[HTTPFlow]) -> List[Dict[str, Any]]:
    ...
    for flow in flows:
        if (flow.request and 
            flow.request.pretty_host and 
            ('meijer.com' in flow.request.pretty_host or 'api.meijer.com' in flow.request.pretty_host)):
            ...
            api_call = {
                ...
                "auth_type": "Bearer" if auth_header.startswith('Bearer ') else "Other" if auth_header else "None",
                "headers": headers
            }
...
def find_valid_bearer_token(bearer_requests: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
...
def main():
    ...
    print(f"🏪 Found {len(meijer_calls)} Meijer API calls ({len(meijer_auth_calls)} with auth)")
    
    if bearer_requests:
        print(f"\n📋 Bearer Token Summary (newest first):")
        for i, request in enumerate(bearer_requests[:10]):  # Show first 10
            status = f" ({request.get('status_code')})" if request.get('status_code') else ""
            print(f"   {i+1}. {request['method']} {request['host']}{request['path'][:50]}...{status}")
            print(f"      Token: {request['bearer_token'][:20]}...{request['bearer_token'][-10:]}")
        ...
        if valid_token:
            ...
            print(f"   Token: {valid_token['bearer_token'][:30]}...{valid_token['bearer_token'][-15:]}")
    ...
    if meijer_auth_calls:
        ...
        for i, call in enumerate(meijer_auth_calls[:5]):  # Show first 5
            ...
            print(f"   {i+1}. {call['method']} {call['host']}{call['path'][:60]}...{status}")
