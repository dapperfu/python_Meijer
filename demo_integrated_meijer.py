...
def demo_comprehensive_integration():
    ...
    print(f"   - MeijerCoupon: {MeijerCoupon.__doc__.split('.')[0] if MeijerCoupon.__doc__ else 'Coupon management'}")
    print(f"   - MeijerItem: {MeijerItem.__doc__.split('.')[0] if MeijerItem.__doc__ else 'Search result items'}")
    print(f"   - MeijerStore: {MeijerStore.__doc__.split('.')[0] if MeijerStore.__doc__ else 'Store information'}")
...
def demo_api_coverage():
    ...
    print("   🏪 Store Proximity: https://api.meijer.com/digital/storeInfo/v2/stores/proximity")
