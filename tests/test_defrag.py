...
def test_defrag() -> None:
    ...
    try:
        ...
        for i, item in enumerate(current_items, 1):
            ...
            print(
                f"  {i:2d}. {item.name:<25} | Qty: {item.quantity} | Notes: {notes_preview}"
            )
        ...
        for i, item in enumerate(defragged_items, 1):
            ...
            notes_preview = (
                item.notes[:40] + "..."
                if item.notes and len(item.notes) > 40
                else item.notes or "None"
            )
            print(
                f"  {i:2d}. {item.name:<25} | Aisle: {aisle_info:<8} | Qty: {item.quantity}"
            )
        ...
        if aisle_summary:
            ...
            sorted_aisles = sorted(
                aisle_summary.items(), key=lambda x: (x[0] == "Unknown", x[0])
            )
            for aisle, count in sorted_aisles:
                print(f"   📍 Aisle {aisle}: {count} item(s)")

        print("\n🎉 Defrag test completed successfully!")
        print("💡 Your shopping list is now organized for efficient store navigation!")
        print("🔍 Check the enhanced notes for detailed product matching information!")
