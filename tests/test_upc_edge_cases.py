...
class TestUPCEdgeCases:
    ...
    def setup_method(self):
        ...
        if self.use_live_endpoints:
            ...
            try:
                ...
                print(
                    f"⚠️  Error initializing live Meijer client: {e}, falling back to mocks"
                )
                self.use_live_endpoints = False
                self._setup_mock_client()
        else:
        ...
        if self.use_live_endpoints:
            ...
            if result:
                ...
                if result.tags:
                    ...
                    has_clearance_tag = any(
                        indicator in " ".join(result.tags).lower()
    ...
    def test_bogo_40_percent_off(self, upc: str):
        ...
        cart_response_data = {
            "items": [
                {"barcode": upc, "quantity": 1, "price": 10.00, "total": 10.00},
                {
                    "barcode": upc,
                    "quantity": 1,
    ...
    def test_upc_edge_case_integration(self):
        ...
        quantity_item = self._create_mock_meijer_item(
            "629307040245", price=3.99, tags=["quantity-discount"]
        )
        for _ in range(5):
        ...
        expected_total = (
            clearance_total + bogo_total + half_clearance_total + quantity_total
        )
