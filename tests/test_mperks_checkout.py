...
class TestMPerksCheckout:
    ...
    def test_get_point_balance(self):
        ...
        try:
            ...
            if balance is None:
                print("⚠️ Point balance returned None - API may not be working or endpoint not implemented")
            ...
            if hasattr(self, 'fuel_coupon_points'):
                if balance.total_points >= self.fuel_coupon_points:
                    print(f"✅ Sufficient points to claim fuel reward ({self.fuel_coupon_points} needed)")
                else:
                    print(f"❌ Insufficient points to claim fuel reward ({self.fuel_coupon_points} needed, {balance.total_points} available)")
    ...
    def test_attempt_fuel_reward_claim(self):
        ...
        try:
            ...
            if any(phrase in error_msg for phrase in [
                "not enough points", "insufficient points", "insufficient balance",
                "points required", "balance too low", "not enough"
            ]):
            ...
            # Check if it's a "not available" or "expired" error
                ...
                "not available", "expired", "no longer available", "unavailable",
            ...
            # Check if it's a "already claimed" error
                ...
                "already claimed", "already purchased", "already redeemed",
        ...
        try:
            ...
            if earn_data.total_all > 0:
                ...
                for offer in earn_data.all_offers[:3]:
                    print(f"   • {offer.title} ({offer.status}) - {offer.points_earned} points")
