...
class CouponPaginationTester:
    ...
    def test_high_limit_single_request(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"method": "Single Request", "success": False, "error": str(e)}
    ...
    def test_pagination_approach(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"method": "Pagination", "success": False, "error": str(e)}
    ...
    def test_coupon_quality(self, coupons: List) -> Dict[str, Any]:
        ...
        has_description = len(
            [c for c in coupons if c.description and c.description.strip()]
        )
        has_amount = len(
            [c for c in coupons if c.redeem_amount and c.redeem_amount > 0]
        )
        has_dates = len(
            [c for c in coupons if c.redemption_start_date or c.redemption_end_date]
        )
        ...
        amounts = [
            c.redeem_amount for c in coupons if c.redeem_amount and c.redeem_amount > 0
        ]
        ...
        result = {
            ...
            "quality_score": (valid_ids + has_title + has_amount)
            / (total_coupons * 3)
            * 100
            if total_coupons > 0
            else 0,
    ...
    def show_sample_coupons(self, coupons: List, count: int = 10):
        ...
        for i, coupon in enumerate(coupons[:count], 1):
            ...
            title = (
                coupon.title[:40] + "..." if len(coupon.title) > 40 else coupon.title
            )
            amount = (
                f"${coupon.redeem_amount:.2f}" if coupon.redeem_amount else "No amount"
            )
            ...
            if coupon.description:
                desc = (
                    coupon.description[:60] + "..."
                    if len(coupon.description) > 60
                    else coupon.description
                )
                print(f"     Desc: {desc}")
    ...
    def run_comprehensive_test(self):
        ...
        if single_result.get("success") and single_result.get("coupons_fetched", 0) > 0:
            ...
            pagination_result.get("success")
            and pagination_result.get("coupons_fetched", 0) > 0
        ):
        ...
        if single_result.get("success"):
            print(
                f"   • Single Request: {single_result['coupons_fetched']} coupons in {single_result['duration_seconds']:.2f}s"
            )
        if pagination_result.get("success"):
            print(
                f"   • Pagination: {pagination_result['coupons_fetched']} coupons in {pagination_result['duration_seconds']:.2f}s"
            )
        ...
        if best_count >= 400:
            print(
                f"   ✅ High-limit fetching working! Getting {best_count} coupons successfully."
            )
            print(
                "   ✅ Use get_coupons(limit=1000) to fetch all ~473 available coupons."
            )
        else:
            print(
                f"   ⚠️  Only getting {best_count} coupons. May need pagination improvements."
            )
