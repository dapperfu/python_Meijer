package com.scandit.datacapture.barcode.internal.module.count.ui.viewholders;

import android.content.Context;
import android.view.View;
import android.widget.RelativeLayout;
import com.scandit.datacapture.barcode.internal.module.count.ui.viewholders.C13139t0;
import com.scandit.datacapture.barcode.internal.sdk.count.ui.BarcodeCountViewDefaults;
import kotlin.jvm.functions.Function0;
import kotlin.jvm.internal.Intrinsics;

/* renamed from: com.scandit.datacapture.barcode.internal.module.count.ui.viewholders.t0, reason: case insensitive filesystem */
/* loaded from: classes11.dex */
public final class C13139t0 implements InterfaceC13126m0 {

    /* renamed from: a, reason: collision with root package name */
    public final BarcodeCountViewUiLayoutHelper f122073a;

    /* renamed from: b, reason: collision with root package name */
    public boolean f122074b;

    /* renamed from: c, reason: collision with root package name */
    public boolean f122075c;

    /* renamed from: d, reason: collision with root package name */
    public Function0 f122076d;

    /* renamed from: e, reason: collision with root package name */
    public com.scandit.datacapture.barcode.internal.module.count.ui.buttons.c f122077e;

    /* renamed from: f, reason: collision with root package name */
    public String f122078f;

    /* renamed from: g, reason: collision with root package name */
    public final z1 f122079g;

    public static final void a(C13139t0 this$0, View view) {
        Intrinsics.j(this$0, "this$0");
        this$0.f122076d.invoke();
    }

    public C13139t0(C13115h layoutHelper, RelativeLayout container) {
        Intrinsics.j(layoutHelper, "layoutHelper");
        Intrinsics.j(container, "container");
        this.f122073a = layoutHelper;
        BarcodeCountViewDefaults barcodeCountViewDefaults = BarcodeCountViewDefaults.INSTANCE;
        barcodeCountViewDefaults.getExitButtonContentDescription();
        this.f122076d = C13134q0.f122064a;
        this.f122078f = barcodeCountViewDefaults.getRestartButtonText();
        this.f122079g = new z1(container);
    }

    @Override // com.scandit.datacapture.barcode.internal.module.count.ui.viewholders.InterfaceC13105c
    public final void b() {
        this.f122079g.b(this.f122077e, new C13130o0(this));
    }

    public final RelativeLayout.LayoutParams c() {
        Context context = ((RelativeLayout) this.f122079g.f122115a).getContext();
        Intrinsics.i(context, "getContext(...)");
        if (com.scandit.datacapture.barcode.internal.module.extensions.b.a(context)) {
            RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(AbstractC13103b.f121963a, AbstractC13103b.f121964b);
            layoutParams.addRule(11);
            layoutParams.addRule(12);
            layoutParams.bottomMargin = this.f122073a.g();
            layoutParams.setMarginEnd(this.f122073a.a());
            return layoutParams;
        }
        RelativeLayout.LayoutParams layoutParams2 = new RelativeLayout.LayoutParams(AbstractC13103b.f121963a, AbstractC13103b.f121964b);
        layoutParams2.addRule(9);
        layoutParams2.addRule(12);
        layoutParams2.setMarginStart(this.f122073a.g());
        layoutParams2.bottomMargin = this.f122073a.a();
        return layoutParams2;
    }

    public final com.scandit.datacapture.barcode.internal.module.count.ui.buttons.c a(Context context) {
        com.scandit.datacapture.barcode.internal.module.count.ui.buttons.c cVar = new com.scandit.datacapture.barcode.internal.module.count.ui.buttons.c(context);
        cVar.setLayoutParams(c());
        cVar.setVisibility(4);
        cVar.setOnClickListener(new View.OnClickListener() { // from class: Pt.i
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                C13139t0.a(this.f29332a, view);
            }
        });
        String str = this.f122078f;
        if (str == null) {
            cVar.f121509b.setText(BarcodeCountViewDefaults.INSTANCE.getRestartButtonText());
        } else {
            cVar.f121509b.setText(str);
        }
        cVar.setContentDescription(BarcodeCountViewDefaults.INSTANCE.getExitButtonContentDescription());
        this.f122077e = cVar;
        return cVar;
    }

    @Override // com.scandit.datacapture.barcode.internal.module.count.ui.viewholders.InterfaceC13105c
    public final void a() {
        com.scandit.datacapture.barcode.internal.module.count.ui.buttons.c cVar = this.f122077e;
        if (cVar != null) {
            cVar.setVisibility(this.f122074b ? 0 : 4);
        }
        com.scandit.datacapture.barcode.internal.module.count.ui.buttons.c cVar2 = this.f122077e;
        if (cVar2 != null) {
            cVar2.setEnabled(this.f122075c);
        }
        com.scandit.datacapture.barcode.internal.module.count.ui.buttons.c cVar3 = this.f122077e;
        if (cVar3 == null) {
            return;
        }
        cVar3.setLayoutParams(c());
    }
}
