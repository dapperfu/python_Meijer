...
class L0 extends CameraCaptureSession.CaptureCallback {
    ...
    final Map<CaptureRequest, List<CameraCaptureSession.CaptureCallback>> f163740a = new HashMap();
    ...
    private List<CameraCaptureSession.CaptureCallback> b(CaptureRequest captureRequest) {
        List<CameraCaptureSession.CaptureCallback> list = this.f163740a.get(captureRequest);
        return list != null ? list : Collections.EMPTY_LIST;
    }

    void a(CaptureRequest captureRequest, List<CameraCaptureSession.CaptureCallback> list) {
        List<CameraCaptureSession.CaptureCallback> list2 = this.f163740a.get(captureRequest);
        if (list2 == null) {
            this.f163740a.put(captureRequest, list);
            return;
        }
        ArrayList arrayList = new ArrayList(list.size() + list2.size());
        arrayList.addAll(list);
        arrayList.addAll(list2);
        this.f163740a.put(captureRequest, arrayList);
    }
    ...
    public void onCaptureSequenceAborted(CameraCaptureSession cameraCaptureSession, int i10) {
        Iterator<List<CameraCaptureSession.CaptureCallback>> it = this.f163740a.values().iterator();
        while (it.hasNext()) {
    ...
    public void onCaptureBufferLost(CameraCaptureSession cameraCaptureSession, CaptureRequest captureRequest, Surface surface, long j10) {
        ...
        while (it.hasNext()) {
            C17672c.a(it.next(), cameraCaptureSession, captureRequest, surface, j10);
        }
    }
