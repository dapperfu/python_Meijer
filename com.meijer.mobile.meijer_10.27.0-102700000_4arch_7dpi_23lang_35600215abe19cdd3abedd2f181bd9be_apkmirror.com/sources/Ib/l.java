package Ib;

import java.util.Map;
import nt.InterfaceC15945c;

/* loaded from: classes4.dex */
public final class l<T> extends com.gimbal.internal.rest.context.a<T> {

    /* renamed from: d, reason: collision with root package name */
    private final h f13796d;

    @Override // com.gimbal.internal.rest.context.a
    protected final T b(String str, Map<String, String> map, Object obj, Class<T> cls) {
        return (T) this.f13796d.c(str, obj, cls);
    }

    public l(h hVar) {
        this.f13796d = hVar;
    }

    public final void i(String str, Object obj, Class<T> cls, InterfaceC15945c<T> interfaceC15945c) {
        d(str, obj, cls, interfaceC15945c);
    }
}
