...
class CriticalSecurityTester:
    ...
    def test_settings_endpoint_vulnerability(self):
        ...
        for account_id in test_accounts:
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    logger.warning(f"⚠️  SETTINGS ENDPOINT VULNERABILITY: Got data for account {account_id}")
                ...
                logger.error(f"Error testing settings endpoint for account {account_id}: {e}")
                results[account_id] = {"error": str(e)}
        ...
        for account_id in test_accounts:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data and isinstance(data, dict) and len(data) > 0:
                        logger.warning(f"⚠️  VEHICLE INFO VULNERABILITY: Got data for account {account_id}")
                ...
                logger.error(f"Error testing vehicle info for account {account_id}: {e}")
                results[account_id] = {"error": str(e)}
    ...
    def _extract_sensitive_info(self, data: Any) -> Dict[str, Any]:
        ...
        if isinstance(data, dict):
            sensitive_fields = [
                ...
                "mPerksId", "mPerksPhone", "loyaltyCardNumber", "employeeId"
            ]
    ...
    def run_critical_tests(self):
        ...
        logger.info("✅ Critical security testing completed. Results saved to critical_security_results.json")
        return results
            ...
            for key, value in test_results.items():
                if isinstance(value, dict) and "vulnerable" in value:
                    if value["vulnerable"]:
                        ...
                        if "cross_account_access" in value and value["cross_account_access"]:
                            print(f"    ⚠️  Cross-account access possible!")
                    else:
                        print(f"  {key}: 🟢 Secure")
