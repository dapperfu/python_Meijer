            ...
            try:
                ...
                if search_results and search_results.results:
                    ...
                    if hasattr(result, 'raw_data'):
                        ...
                        location_fields = ['aisle', 'section', 'bay', 'location', 'aisleLocation', 
                                         'storeLocation', 'productLocation', 'itemLocation', 'store']
                        found_fields = []
                        for field in location_fields:
                            if field in result.raw_data:
                                found_fields.append(f"{field}: {result.raw_data[field]}")
                    ...
                    if result.upc:
                        ...
                        if product_detail:
                            logger.info(f"   Product detail found: {product_detail.title}")
                            logger.info(f"   Aisle primary: {getattr(product_detail, 'aisle_primary', 'None')}")
                            logger.info(f"   Aisle locations: {getattr(product_detail, 'aisle_locations', 'None')}")
                        else:
