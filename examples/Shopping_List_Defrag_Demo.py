...
def create_defrag_demo_notebook() -> Dict[str, Any]:
    ...
    notebook["cells"].append({
        ...
        "source": [
            ...
            "\n",
            "if defragged_items:\n",
            "    # Display defragged items in a table\n",
            "    defragged_data = []\n",
            "    \n",
            "    for i, item in enumerate(defragged_items, 1):\n",
            "        # Extract aisle info from notes if available\n",
            "        aisle_info = 'Unknown'\n",
            ...
            "            'Order': i,\n",
            "            'Item': item.name[:35],  # Truncate long names\n",
            "            'Aisle': aisle_info,\n",
            "            'Quantity': item.quantity,\n",
            "            'Location Notes': item.notes[:50] + '...' if item.notes and len(item.notes) > 50 else item.notes or 'None',\n",
            "            'Status': '✅ Done' if item.checked else '⏳ Pending'\n",
            "        })\n",
            "    \n",
            "    df_defragged = pd.DataFrame(defragged_data)\n",
            "    print(\"\\n📊 Defragged Shopping List (Organized by Aisle):\")\n",
