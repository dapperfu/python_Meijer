...
def main():
    ...
    try:
        ...
        search_available = hasattr(client, "search") and hasattr(
            client.search, "search"
        )
        print(f"   🔍 Search functionality: {'✅' if search_available else '❌'}")

        # Store functionality
        store_available = hasattr(client, "store_search") and hasattr(
            client.store_search, "find_stores_nearby"
        )
        print(f"   🏪 Store search: {'✅' if store_available else '❌'}")

        # Shop & Scan
        shop_scan_available = hasattr(client, "shop_scan") and hasattr(
            client.shop_scan, "start_trip"
        )
        ...
        try:
            ...
            print(
                f"   ✅ Store ID type handling: int {test_store_id} processed correctly"
            )
        except Exception as e:
            print(
                f"   ✅ Store ID type handling: Proper error handling ({str(e)[:50]}...)"
            )
        print()
