...
try:
    ...
    print("❌ Error: meijer package not found")
...
class MeijerAdBrowser:
    ...
    def initialize_client(self) -> bool:
        """Initialize the Meijer client and check authentication."""
        try:
            self.meijer_client = Meijer()
            
            # Check if we have authentication
            if not self.meijer_client._access_token:
                self.console.print("❌ No authentication token found", style="red")
                self.console.print("Please run 'meijer auth <log_file>' first", style="yellow")
                return False
                
            self.console.print("✅ Meijer client initialized successfully", style="green")
            return True
            
        except Exception as e:
            self.console.print(f"❌ Failed to initialize Meijer client: {e}", style="red")
            return False
            ...
            for offer in offers:
                if hasattr(offer, 'categoryID') and offer.categoryID == "20250817inad":
                    ad_item = AdItem(
                        ...
                        image_url=getattr(offer, 'imageUrl', None),
