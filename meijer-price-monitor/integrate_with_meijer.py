...
def create_authenticated_monitor() -> Optional[PriceMonitor]:
    ...
    try:
        ...
        if not meijer.is_authenticated():
            ...
            print(
                "   You can also use the standalone monitor without authentication for testing."
            )
            return None
...
def demonstrate_monitor_creation(price_monitor: PriceMonitor) -> None:
    ...
    try:
        ...
        for mid, config in monitors.items():
            print(
                f"  • {config['name']}: {config['search_query']} in {config['location']}"
            )
    ...
    try:
        ...
        if deals:
            ...
            for i, deal in enumerate(deals[:5], 1):  # Show first 5
                print(
                    f"   {i}. Product {deal['product_id']} at store {deal['store_id']}"
                )
                print(
                    f"      Price: ${deal['oldest_price']:.2f} → ${deal['current_price']:.2f}"
                )
                print(
                    f"      Discount: {deal['discount_percent']:.1f}% (${deal['price_drop']:.2f} savings)"
                )
                print()
        else:
            print(
                "ℹ️  No clearance deals found yet. Run more monitors to build price history."
            )
