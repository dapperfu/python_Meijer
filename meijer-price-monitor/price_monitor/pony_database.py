...
class PonyPriceDatabase:
        ...
        try:
            with self._get_session():
                products = select(
                    ...
                    or (p.brand and query.lower() in p.brand.lower())
    ...
    def get_product_prices_by_upc(self, upc: str) -> List[Dict[str, Any]]:
        ...
        try:
            with self._get_session():
                price_records = select(
                    pr for pr in PriceRecord if pr.product.upc == upc
    ...
    def get_price_history(
        ...
        try:
            with self._get_session():
                ...
                price_records = select(
                    pr
                    for pr in PriceRecord
                    if pr.product.upc == upc
                    and pr.store.store_id == store_id
                    and pr.timestamp >= cutoff_date
                ).order_by(desc(PriceRecord.timestamp))
    ...
    def find_price_drops(
        ...
        try:
            with self._get_session():
                ...
                price_records = select(
                    pr for pr in PriceRecord if pr.timestamp >= cutoff_date
                ).order_by(desc(PriceRecord.timestamp))
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self._get_session():
                ...
                stats["shopnscan_verifications_count"] = count(
                    sv for sv in ShopnScanVerification
                )
