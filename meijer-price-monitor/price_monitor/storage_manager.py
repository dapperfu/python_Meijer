...
class DualPathStorageManager:
    ...
    def store_search_results(
        ...
        search_id = self.database.add_search_query(
            query_text_for_db, len(results), len(query.stores) if query.stores else 0
        )
        ...
        json_filename = (
            f"search_{timestamp_str}_{query_text.replace(' ', '_')[:20]}.json"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Search results stored: {len(results)} results in {json_filename}"
            )
    ...
    def store_price_records(
        self, price_records: List[PriceRecord], source: str = "search"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Price records stored: {len(price_records)} records in {json_filename}"
            )
    ...
    def store_shopnscan_verifications(
        self, verifications: List[ShopnScanPrice]
    ) -> Dict[str, Any]:
        """
        ...
        json_filepath = (
            self.data_dir / "json" / "shopnscan_verifications" / json_filename
        )
        ...
        try:
            ...
            self.logger.info(
                f"Shop'n'Scan verifications stored: {len(verifications)} in {json_filename}"
            )
    ...
    def store_monitor_run(
        self, monitor_id: str, monitor_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for file_path in files_to_remove:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to remove old file {file_path.name}: {e}"
                    )
