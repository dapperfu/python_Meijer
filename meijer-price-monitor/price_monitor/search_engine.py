...
class EnhancedSearchEngine:
    ...
    def __init__(self, meijer_client: Optional[Any] = None):
        ...
        if MEIJER_AVAILABLE and meijer_client:
            ...
            self.stores = None
            self.logger.warning(
                "Meijer API not available. Using mock search functionality."
            )

    def search_products(
        self, query: SearchQuery, progress_callback: Optional[callable] = None
    ) -> List[SearchResult]:
        """
        ...
        if progress_callback:
            progress_callback(
                f"Searching {len(stores_to_search)} stores for '{query.query_text}'"
            )
        ...
        for i, store_id in enumerate(stores_to_search):
            try:
                if progress_callback:
                    progress_callback(
                        f"Searching store {i + 1}/{len(stores_to_search)}: {store_id}"
                    )
                ...
                if len(all_results) >= query.max_results:
                    self.logger.info(
                        f"Reached maximum results limit ({query.max_results})"
                    )
                    break
            ...
            try:
                ...
                for item in page_results:
                    search_result = self._convert_to_search_result(
                        item, store_id, query.query_text
                    )
                    if search_result:
                ...
                if len(store_results) >= query.min_results and not self._has_more_pages(
                    page_results
                ):
                    break
                ...
                self.logger.error(
                    f"Failed to search page {page} at store {store_id}: {e}"
                )
                break
    ...
    def _search_page(
        self, query_text: str, store_id: str, page: int, results_per_page: int
    ) -> List[Any]:
        """
    ...
    def _convert_to_search_result(
        self, item: Any, store_id: str, search_query: str
        ...
        try:
            ...
            if not upc:
                self.logger.warning(
                    f"Could not extract UPC from item: {getattr(item, 'name', 'Unknown')}"
                )
                return None
    ...
    def _get_available_stores(self) -> List[str]:
        ...
        if not self.stores:
            raise RuntimeError(
                "Meijer stores not available. Please ensure Meijer API is properly initialized."
            )
    ...
    def verify_prices_with_shopnscan(
        ...
        """
        self.logger.info(
            f"Verifying prices for {len(upcs)} products at store {store_id}"
        )
