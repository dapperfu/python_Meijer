...
def test_upc_search():
    ...
    if results:
        ...
        for i, result in enumerate(results, 1):
            ...
            print(
                f"     Status: {'Clearance' if result.is_clearance else 'On Sale' if result.is_on_sale else 'Regular'}"
            )
...
def test_backup_and_export():
    ...
    export_result = monitor.export_data(
        "price_drops", {"min_drop_percent": 5.0, "days": 7}
    )
...
def main():
    ...
    try:
        ...
        if search_results:
            ...
            upc_to_test = (
                "822279082910"  # Use the original UPC if search didn't find it
            )
        ...
        print("\n📋 Test Summary:")
        print(
            f"  • Enhanced search: ✅ {len(search_results) if search_results else 0} products found"
        )
        print(
            f"  • Price verification: ✅ Tested across {len(verification_results)} stores"
        )
        print("  • Storage statistics: ✅ Retrieved successfully")
        print("  • Database operations: ✅ All operations completed")
        print(
            f"  • Backup creation: ✅ {'Success' if backup_result['status'] == 'success' else 'Failed'}"
        )
        print(
            f"  • Data export: ✅ {'Success' if export_result['export_file'] else 'Failed'}"
        )
