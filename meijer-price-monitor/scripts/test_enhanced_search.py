...
def test_enhanced_search():
    ...
    if results:
        ...
        print(
            f"  • Brands: {', '.join(f'{brand}: {count}' for brand, count in brands.items())}"
        )
...
def main():
    ...
    try:
        ...
        if search_results:
            verification_results = test_price_verification(
                [r.upc for r in search_results]
            )
        ...
        print(
            f"  • Price verification: ✅ Tested with {len(search_results[:3]) if search_results else 0} UPCs"
        )
        print("  • Storage statistics: ✅ Retrieved successfully")
