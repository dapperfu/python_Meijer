...
def migrate_search_queries(
    old_db_path: Path, new_db: PonyPriceDatabase
) -> Dict[int, int]:
    """
...
def migrate_search_results(
    old_db_path: Path, new_db: PonyPriceDatabase, id_mapping: Dict[int, int]
) -> int:
    """
        ...
        with sqlite3.connect(str(old_db_path)) as conn:
            ...
            for result_row in results:
                ...
                if new_search_id is None:
                    logger.warning(
                        f"Skipping search result with unknown search_id: {old_search_id}"
                    )
                    continue
                ...
                if new_db.add_search_result(new_search_id, result_data):
                    ...
                    logger.warning(
                        f"Failed to migrate search result: {result_row['id']}"
                    )
...
def migrate_shopnscan_verifications(
    old_db_path: Path, new_db: PonyPriceDatabase
) -> int:
    """
        ...
        with sqlite3.connect(str(old_db_path)) as conn:
            ...
            for verification_row in verifications:
                ...
                if new_db.add_shopnscan_verification(verification):
                    ...
                    logger.warning(
                        f"Failed to migrate verification: {verification_row['id']}"
                    )

            logger.info(
                f"Successfully migrated {migrated_count} Shop'n'Scan verifications"
            )
            return migrated_count
...
def main():
    ...
    try:
        ...
        search_results_migrated = migrate_search_results(
            old_db_path, new_db, id_mapping
        )
