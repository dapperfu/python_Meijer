...
class MeijerAdBrowser:
    ...
    def load_weekly_ad_items(self) -> bool:
        ...
        try:
            ...
            self.console.print(
                f"✅ Loaded {len(ad_items)} weekly ad items", style="green"
            )
    ...
    def load_shopping_list_status(self):
        ...
        try:
            ...
            self.console.print(
                f"⚠️ Could not load shopping list status: {e}", style="yellow"
            )
    ...
    def display_navigation(self):
        ...
        if self.selected_items:
            nav_text += (
                f" | [a]dd {len(self.selected_items)} selected, [c]lear selection"
            )

        nav_panel = Panel(Align.center(nav_text), box=box.ROUNDED, style="cyan")
    ...
    def display_help(self):
        ...
        help_panel = Panel(help_text, title="Help", box=box.ROUNDED, style="green")
    ...
    def add_selected_to_shopping_list(self):
        ...
        selected_items = [
            item for item in self.ad_items if item.offer_id in self.selected_items
        ]

        self.console.print(
            f"🔄 Adding {len(selected_items)} items to shopping list...", style="yellow"
        )
        ...
        for item in selected_items:
            try:
                ...
                if item.is_in_list:
                    self.console.print(
                        f"⚠️ {item.name} is already in shopping list", style="yellow"
                    )
                    continue
    ...
    def remove_from_shopping_list(self, item_name: str):
        ...
        try:
            ...
            for item in self.meijer_client.list.get():
                if (
                    hasattr(item, "item_description")
                    and item.item_description.lower() == item_name.lower()
                ):
                    list_item = item
                    break
            ...
            if not list_item:
                self.console.print(
                    f"⚠️ {item_name} not found in shopping list", style="yellow"
                )
                return False
            ...
            if self.meijer_client.list.delete_item(str(list_item.list_item_id)):
                self.console.print(
                    f"✅ Removed {item_name} from shopping list", style="green"
                )
            ...
            try:
                ...
                if choice == "q":
                    ...
                    try:
                        page_num = int(
                            Prompt.ask(
                                "Enter page number", default=str(self.current_page + 1)
                            )
                        )
                        if 1 <= page_num <= self.get_total_pages():
                            ...
                            self.console.print(
                                f"⚠️ Invalid page number. Must be 1-{self.get_total_pages()}",
                                style="yellow",
                            )
                    except ValueError:
