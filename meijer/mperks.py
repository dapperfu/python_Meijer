...
class MPerksClient:
    ...
    def _make_request(
        ...
        try:
            ...
            if "application/vnd.meijer.digitalmperks" in response.headers.get(
                "content-type", ""
            ):
        ...
        try:
            ...
            try:
                ...
                if response and isinstance(response, dict):
                    ...
                    self.logger.info(
                        "Successfully fetched history from history endpoint"
                    )
            except Exception as e:
            ...
            if not history_data:
                try:
                    ...
                    if response and isinstance(response, dict):
                        history_data = response
                        self.logger.info(
                            "Successfully fetched history from transactions endpoint"
                        )
                except Exception as e:
            ...
            if not history_data:
                try:
                    ...
                    if response and isinstance(response, dict):
                        history_data = response
                        self.logger.info(
                            "Successfully fetched history from activity endpoint"
                        )
                except Exception as e:
            ...
            if not history_data:
                self.logger.warning(
                    "All history endpoints failed, returning empty history"
                )
                return MPerksHistory()
    ...
    def _create_history_event_from_data(
        self, event_data: Dict[str, Any]
    ) -> Optional[MPerksHistoryEvent]:
        """
        ...
        try:
            ...
            date_str = (
                event_data.get("date")
                or event_data.get("transactionDate")
                or event_data.get("earnDate")
                or event_data.get("timestamp")
            )
            points_change = (
                event_data.get("points")
                or event_data.get("pointsChange")
            ...
            description = (
                ...
                or event_data.get("details")
                or event_data.get("note")
            )
            transaction_id = (
            ...
            if any(
                word in event_type_lower
                for word in ["purchase", "earn", "earned", "shopping"]
            ):
                event_type = "Purchase Earnings"
            elif any(
                word in event_type_lower
                for word in ["reward", "claim", "claimed", "redeem", "redemption"]
            ):
                event_type = "Reward Claimed"
            elif any(
                word in event_type_lower for word in ["bonus", "promotion", "offer"]
            ):
                event_type = "Bonus Points"
            elif any(
                word in event_type_lower for word in ["expire", "expired", "forfeit"]
            ):
    ...
    def _create_reward_from_api_data(
        self, reward_data: Dict[str, Any]
    ) -> Optional[BaseReward]:
        """
        ...
        try:
            ...
            reward = self._determine_reward_type_and_create(
                reward_data, name, description
            )
    ...
    def _create_earned_reward_from_api_data(
        self, reward_data: Dict[str, Any]
    ) -> Optional[BaseReward]:
        """
        ...
        try:
            ...
            if reward:
                ...
                if earn_date:
                    try:
                        reward.metadata.earn_date = datetime.fromisoformat(
                            earn_date.replace("Z", "+00:00")
                        )
                    except:
                        pass
                ...
                if expiration_date:
                    try:
                        reward.metadata.expiration_date = datetime.fromisoformat(
                ...
                self.logger.debug(
                    f"Created earned reward: {reward.title} (ID: {meijer_offer_id})"
                )
                return reward
    ...
    def _determine_reward_type_and_create(
        self, reward_data: Dict[str, Any], title: str, description: str
    ) -> Optional[BaseReward]:
        """
    ...
    def _create_fuel_reward(
        ...
        points_required = self._estimate_points_for_fuel_reward(
            discount_per_gallon, max_gallons
        )
    ...
    def _estimate_points_for_fuel_reward(
        self, discount_per_gallon: Decimal, max_gallons: Optional[int]
    ) -> int:
