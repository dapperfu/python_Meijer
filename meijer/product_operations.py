...
class ProductOperations:
    ...
    def get_product_detail(self, upc: str, store_id: Optional[str] = None) -> Optional[MeijerItem]:
        ...
        try:
            ...
            search_results = self.client.search.search_products(upc, limit=1)
            ...
            if search_results and search_results.results:
                ...
                if store_id and hasattr(self.client, 'get_store_by_id'):
                    ...
                    if store:
                        self.logger.debug(f"Found store {store.name} for product lookup")
        ...
        try:
            ...
            if location_info:
                ...
                if location_info.get("section"):
                    item.aisle_locations.append(f"{location_info['aisle']} {location_info['section']}")
    ...
    def _extract_location_from_product_detail(self, data: Dict[str, Any]) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            location_fields = [
                "aisle", "section", "bay", "location", "aisleLocation",
                "storeLocation", "productLocation", "itemLocation"
            ]
    ...
    def _extract_location_from_text(self, text: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            location_match = re.search(r'([A-Z]\d+)[-\s]+([A-Z]\d+)[-\s]+([A-Z]\d+)', text)
            if location_match:
                return {
