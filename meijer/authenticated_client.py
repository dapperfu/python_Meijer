...
class AuthenticatedMeijerClient:
    ...
    def _capture_initial_cookies(self) -> Dict[str, str]:
        ...
        try:
                    ...
                    try:
                        ...
                        try:
                            login_elements = self.driver.find_elements(
                                "xpath",
                                "//*[contains(text(), 'Sign In') or contains(text(), 'Login') or contains(@class, 'login') or contains(@id, 'login')]",
                            )
                            if login_elements:
                                logger.info(
                                    "Found login elements, clicking to trigger more activity"
                                )
            ...
            for cookie_name in cookies:
                if any(
                    prefix in cookie_name for prefix in ["ak_", "bm_", "JSESSIONID"]
                ):
                    logger.info(
                        f"Cookie: {cookie_name} = {cookies[cookie_name][:50]}..."
                    )
            ...
            if missing_cookies:
                ...
                akamai_cookies = {
                    ...
                    if k.startswith("ak_") or k.startswith("bm_")
                }
                if akamai_cookies:
                    logger.info(
                        f"Have some Akamai cookies: {list(akamai_cookies.keys())}"
                    )
    ...
    def _submit_username(
        self, username: str, cookies: Dict[str, str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = self.session.post(
                f"{self.okta_base}/idp/idx/identify", json=identify_data, timeout=30
            )

            if response.status_code != 200:
                raise AuthenticationError(
                    f"Identify request failed: {response.status_code}"
                )
        ...
        try:
            ...
            if "tokens" in response_data:
                ...
                if "accessToken" in tokens:
                    ...
                    auth_data["token_type"] = tokens["accessToken"].get(
                        "tokenType", "Bearer"
                    )
            ...
            if "authorization" in self.session.headers:
                ...
                if auth_header.startswith("Bearer "):
                    auth_data["bearer_token"] = auth_header[
                        7:
    ...
    def _submit_2fa_code(self, code: str) -> Dict[str, Any]:
        ...
        try:
            if not self.state_handle:
                raise AuthenticationError(
                    "No state handle available for 2FA verification"
                )
            ...
            if response.status_code != 200:
                raise AuthenticationError(
                    f"2FA verification failed: {response.status_code}"
                )
    ...
    def _initiate_2fa_challenge(self, method: str = "email") -> Dict[str, Any]:
        ...
        try:
            ...
            challenge_data = {
                "authenticator": {"methodType": method, "id": self.authenticator_id},
                "stateHandle": self.state_handle,
            }

            logger.info(f"Initiating {method} 2FA challenge...")
            response = self.session.post(
                f"{self.okta_base}/idp/idx/challenge", json=challenge_data, timeout=30
            )

            if response.status_code != 200:
                raise AuthenticationError(
                    f"2FA challenge initiation failed: {response.status_code}"
                )
    ...
    def _submit_password(self, password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                raise AuthenticationError(
                    f"Password challenge failed: {response.status_code}"
                )
            ...
            if "access_token" in auth_data:
                ...
                try:
                    response = self.session.get(
                        f"{self.meijer_base}/account", timeout=10
                    )
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"Existing token test failed: {response.status_code}"
                        )
                        return False
                except Exception as e:
    ...
    def login(self, username: str, password: str, two_factor_callback=None) -> bool:
        ...
        try:
            ...
            logger.info(
                "No valid existing tokens, proceeding with full authentication..."
            )
            ...
            try:
                ...
                if verification_response.get("type") == "SUCCESS":
                    ...
                    raise AuthenticationError(
                        "2FA verification did not complete successfully"
                    )
    ...
    def _load_auth_tokens(self) -> bool:
        ...
        try:
            ...
            if timestamp > 0:
                ...
                logger.info(
                    f"Current time: {current_time}, time difference: {time_diff}"
                )
            ...
            if "bearer_token" in auth_data:
                self.session.headers["Authorization"] = (
                    f"Bearer {auth_data['bearer_token']}"
                )
...
def create_authenticated_client(
    username: str, password: str, two_factor_callback=None, headless: bool = True
