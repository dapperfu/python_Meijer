...
class EnhancedMeijerAuth:
    ...
    def authenticate(self) -> Optional[AuthTokens]:
        ...
        3. If refresh fails and we have credentials, try widget simulation
        4. If all else fails, prompt user to run 'meijer auth'
        ...
        if stored_tokens and stored_tokens.refresh_token:
            ...
            if self.token_storage.refresh_tokens(stored_tokens.refresh_token):
                refreshed_tokens = self.token_storage.load_tokens()
                if refreshed_tokens:
        ...
        # Step 3: Try JavaScript widget simulation if we have credentials
        if self.username and self.password:
            print("📡 Step 3: Attempting JavaScript widget simulation...")
            ...
            print("💡 Step 3: Skipped widget simulation - no credentials provided")

        # Step 4: All methods failed
        print("❌ All authentication methods failed")
        print("\n💡 Recommendations:")
        print("   1. Run 'meijer auth' to capture tokens from browser login")
        print("   2. Provide username/password for widget simulation")
        print("   3. Check network connectivity and credentials")
