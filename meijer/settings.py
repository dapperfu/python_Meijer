...
class PreferenceDiscreteChoice:
    ...
    def from_api_response(cls, data: Dict[str, Any]) -> "PreferenceDiscreteChoice":
        ...
        return cls(
            digital_preference_discrete_choice_id=data.get("digitalPreferenceDiscreteChoiceId", 0),
            digital_preference_discrete_choice_value=data.get("digitalPreferenceDiscreteChoiceValue", "")
        )
...
class MeijerSettings:
    ...
    def get_vehicle_information(self) -> Optional[VehicleInformation]:
        ...
        try:
            ...
            response = self.meijer._make_request(
                "GET", 
                f"{self.meijer.api_base_url}{self.endpoints['get_vehicle_info']}"
            )
        ...
        try:
            ...
            if vehicle_id is None:
                ...
                if current_vehicle:
                    ...
                    self.logger.error("❌ No current vehicle found and no vehicle_id provided")
                    return False
    ...
    def get_customer_preferences(self) -> List[CustomerPreference]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                preferences = [CustomerPreference.from_api_response(pref) for pref in data]
    ...
    def get_preference_discrete_choices(self) -> List[PreferenceDiscreteChoice]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                choices = [PreferenceDiscreteChoice.from_api_response(choice) for choice in data]
                self.logger.info(f"✅ Found {len(choices)} discrete choices")
                return choices
            else:
                self.logger.warning(f"⚠️  Failed to get discrete choices: {response.status_code}")
                return []
    ...
    def update_customer_preference(
        ...
        """
        try:
            self.logger.info(f"🔧 Updating preference {preference_type_name} to: {preference_value}")
            ...
            if response.status_code == 200:
                self.logger.info(f"✅ Preference {preference_type_name} updated successfully")
                return True
            else:
    ...
    def get_account_details(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.meijer._make_request(
                "GET",
                f"{self.meijer.api_base_url}{self.endpoints['get_account']}"
            )
            ...
            if response.status_code == 200:
                ...
                self.logger.warning(f"⚠️  Failed to get account details: {response.status_code}")
                return None
    ...
    def update_account_details(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.warning(f"⚠️  Failed to update account: {response.status_code}")
                return False
