...
class PriceWatchManager:
    ...
    def add_watch(
        ...
        existing_watch = Watch.get(product=product, store=store, active=True)
    ...
    def remove_watch(self, identifier: str, store_id: Optional[str] = None) -> bool:
        ...
        if store_id:
            ...
            watches = Watch.select(lambda w: w.product == product and w.active)
    ...
    @db_session
    def list_watches(
        self, active_only: bool = True, store_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for watch in query:
            ...
            watch_info = {
                ...
                "current_price_type": current_price.get("price_type")
                if current_price
                else None,
                "last_observed": current_price.get("observed_at")
                if current_price
                else None,
            }
    ...
    def refresh_prices(
        ...
        try:
            ...
            self.logger.info(
                f"Refreshing prices for {len(watches)} watches using {method}"
            )
                ...
                for watch in batch:
                    try:
                        ...
                        if price_info:
                            ...
                            warnings.append(
                                f"Could not fetch price for {watch.product.identifier}"
                            )

                    except Exception as e:
                        error_count += 1
                        warnings.append(
                            f"Error refreshing {watch.product.identifier}: {e}"
                        )
                        self.logger.error(
                            f"Error refreshing {watch.product.identifier}: {e}"
                        )
            ...
            self.logger.info(
                f"Price refresh completed: {refreshed_count} refreshed, {error_count} errors"
            )
    ...
    def generate_alerts(self, since: Optional[datetime] = None) -> List[Dict[str, Any]]:
        ...
        if since is None:
            ...
            last_run = (
                RunLog.select(lambda r: r.command.startswith("refresh_prices") and r.ok)
        ...
        for watch in watches:
            ...
            recent_prices = PriceHistory.select(
                lambda ph: ph.product == watch.product
                and (watch.store is None or ph.store == watch.store)
                and ph.observed_at >= since
            ...
            for current_price in recent_prices:
                ...
                existing_alert = AlertEvent.get(
                    watch=watch, price_history=current_price
                )
    ...
    def _get_current_price(
        self, product: Product, store: Optional[Store]
    ...
    def _fetch_current_price(
        self, identifier: str, id_type: str, store: Optional[Store], method: str
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    def _record_price_history(
        self, product: Product, store: Store, price_info: Dict[str, Any], source: str
    ) -> PriceHistory:
        """
        ...
        existing = PriceHistory.select(
            lambda ph: ph.product == product
            and ph.store == store
            and ph.price == price_info["price"]
            and ph.price_type == price_info["price_type"]
        ).first()
    ...
    def _check_alert_conditions(
        ...
        if current_price.price < previous_price.price:
            ...
            if watch.only_when in [
                OnlyWhenType.ANY_DROP,
                OnlyWhenType.SALE_OR_CLEARANCE,
            ]:
                alerts.append({"reason": AlertReason.DROP, "delta_amount": delta})

        # Check for desired price
        if watch.desired_price and current_price.price <= watch.desired_price:
            delta = current_price.price - previous_price.price
            alerts.append(
                {"reason": AlertReason.HIT_DESIRED_PRICE, "delta_amount": delta}
            )

        # Check for sale detection
        if current_price.price_type == PriceType.SALE and previous_price.price_type in [
            PriceType.REGULAR,
            PriceType.UNKNOWN,
        ]:
            if watch.only_when in [OnlyWhenType.SALE_OR_CLEARANCE]:
                delta = current_price.price - previous_price.price
                alerts.append(
                    {"reason": AlertReason.SALE_DETECTED, "delta_amount": delta}
                )

        # Check for clearance detection
        if (
            current_price.price_type == PriceType.CLEARANCE
            and previous_price.price_type != PriceType.CLEARANCE
        ):
            if watch.only_when in [
                OnlyWhenType.SALE_OR_CLEARANCE,
                OnlyWhenType.CLEARANCE_ONLY,
            ]:
                delta = current_price.price - previous_price.price
                alerts.append(
                    {"reason": AlertReason.CLEARANCE_DETECTED, "delta_amount": delta}
                )
