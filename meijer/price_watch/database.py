...
class PriceWatchDatabase:
    ...
    def _bind_entities(self) -> None:
        ...
        self.db.bind(provider="sqlite", filename=str(self.db_path), create_db=True)
        ...
        try:
            with db_session:
                ...
                if schema_version is None:
                    ...
                    SchemaVersion(
                        version=1, description="Initial schema for price watch system"
                    )
                    ...
                    self.logger.debug(
                        f"Database already initialized (version {schema_version.version})"
                    )
