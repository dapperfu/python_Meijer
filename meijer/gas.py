    ...
    def from_api_data(cls, data: Dict[str, Any], store_id: str) -> "MeijerGas":
        ...
        if "GasStationHours" in data and data["GasStationHours"] is not None:
            try:
                ...
                days_open = [
                    hour.get("DayOfTheWeek", "Unknown")
                    for hour in data["GasStationHours"]
                ]
        ...
        has_convenience_store = any(
            x in amenities for x in ["Beer", "Tobacco", "Gift Cards"]
        )
        ...
        payment_methods = ["Credit Card", "Debit Card", "Cash", "Meijer Gift Card"]
