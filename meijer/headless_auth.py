...
class HeadlessMeijerAuth:
    ...
    def _submit_password(self) -> Dict:
        ...
        if data.get("intent") == "LOGIN" and data.get("success"):
            ...
            if "stateToken=" in success_url:
                self.flow_state.state_token = success_url.split("stateToken=")[1].split(
                    "&"
                )[0]
    ...
    def _exchange_tokens(self) -> Dict:
        ...
        if not self.flow_state.state_token:
            raise MeijerAuthenticationError(
                "No state token available for token exchange"
            )
    ...
    def authenticate(self) -> Dict:
        ...
        try:
            ...
            if (
                password_data.get("intent") == "LOGIN"
                and "success" not in password_data
            ):
                if self.email_2fa_config:
                    ...
                    raise MFARequiredError(
                        "MFA required but no email configuration provided"
                    )
    ...
    def _handle_email_mfa(self) -> Dict:
        ...
        try:
            ...
            raise MFARequiredError(
                "Email MFA verification not yet implemented in headless mode"
            )
...
def authenticate_with_requests(
    ...
    auth = HeadlessMeijerAuth(
        username, password, email_2fa_config, proxy_host, proxy_port
    )
