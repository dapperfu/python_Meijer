    ...
    def __post_init__(self):
        ...
        if isinstance(self.created_date, str):
            self.created_date = datetime.fromisoformat(
                self.created_date.replace("Z", "+00:00")
            )
        if isinstance(self.modified_date, str):
            self.modified_date = datetime.fromisoformat(
                self.modified_date.replace("Z", "+00:00")
            )
...
class CouponCollection:
    ...
    def filter_by_department(self, department: str) -> "CouponCollection":
        ...
        filtered = [
            ...
            if c.department and department.lower() in c.department.lower()
        ]
    ...
    def filter_by_category(self, category: str) -> "CouponCollection":
        ...
        filtered = [
            c
            for c in self.coupons
            if c.category and category.lower() in c.category.lower()
        ]
        return CouponCollection(filtered)
    ...
    def __str__(self) -> str:
        ...
        return (
            f"CouponCollection({total} total, {clipped} clipped, {available} available)"
        )
                ...
                try:
                    ...
                    imported_coupon = Coupon(
                        ...
                        coupon_type=CouponType(
                            coupon_data.get("coupon_type", "digital")
                        ),
                    ...
                    print(
                        f"Error importing coupon {coupon_data.get('id', 'unknown')}: {e}"
                    )
                    continue
        ...
        try:
            ...
            for coupon_data in backup_data["coupons"]:
                try:
                    ...
                    print(
                        f"Error restoring coupon {coupon_data.get('id', 'unknown')}: {e}"
                    )
                    continue
        ...
        try:
            ...
            for coupon_data in shared_data["coupons"]:
                try:
                    ...
                    print(
                        f"Error importing shared coupon {coupon_data.get('id', 'unknown')}: {e}"
                    )
                    continue
