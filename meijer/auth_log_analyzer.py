...
class MeijerAuthLogAnalyzer:
        ...
        try:
            ...
            with open(self.log_file, "rb") as f:
                ...
                for flow in reader.stream():
                    ...
                    if (hasattr(flow, 'request') and 
                        hasattr(flow.request, 'pretty_url') and
                        hasattr(flow.request, 'method')):
    ...
    def detect_full_login_event(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        for flow in self.flows:
                    ...
                    try:
                        ...
                        if 'access_token' in response_data:
                            ...
                            if response_data.get('refresh_token'):
                                logger.info("   Refresh token available for auto-renewal")
                    except (json.JSONDecodeError, AttributeError):
                        pass
            ...
            # API calls (verification that tokens work)
                ...
                logger.debug(f"   Authorization header: {auth_header[:50] if auth_header else 'NOT_FOUND'}...")
        ...
        if token_data.get('bearer_tokens'):
            ...
            token_data['expires_in'] = latest_bearer.get('expires_in', 3600)  # Default to 1 hour
        ...
        has_auth_context = (
            ...
            (login_indicators["oauth2_authorize"] or login_indicators["idx_introspect"]) and
            (login_indicators["login_redirect"] or login_indicators["idx_identify"])
        )
        ...
        if has_auth_context:
            ...
            logger.info("✅ Authentication context detected - OAuth2 parameters available for future use")
            if not token_data.get('access_token'):
                logger.info("⚠️ Authentication flow incomplete - final tokens not captured")
                logger.info("💡 Re-run with complete login flow to capture refresh tokens")
    ...
    def validate_token_refresh_capability(self, tokens: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if not validation['missing_components']:
            ...
            validation['recommendations'].append("✅ Full token refresh capability available")
        else:
            ...
            if 'refresh_token' in validation['missing_components']:
                validation['recommendations'].append("   - No refresh token found - tokens will expire")
            if 'oauth2_parameters' in validation['missing_components']:
                validation['recommendations'].append("   - OAuth2 parameters missing - cannot initiate refresh flow")
            if 'oauth2_client_id' in validation['missing_components']:
                validation['recommendations'].append("   - Client ID missing - required for token refresh")
            if 'oauth2_scope' in validation['missing_components']:
                validation['recommendations'].append("   - Scope missing - required for token refresh")
        
        # Add specific recommendations based on what we found
        if tokens.get('access_token') and not tokens.get('refresh_token'):
            validation['recommendations'].append("   - Bearer token only - use for immediate API access")
            validation['recommendations'].append("   - Re-run with complete login flow to get refresh tokens")
    ...
    def save_tokens_to_auth_json(self, tokens: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if tokens.get('has_auth_context') and not tokens.get('access_token'):
                ...
                logger.info("⚠️ Creating placeholder access token for authentication context")
                logger.info("💡 This file contains OAuth2 parameters but no working tokens")
            ...
            if tokens.get('oauth2_parameters'):
                ...
                logger.info(f"   Client ID: {tokens['oauth2_parameters'].get('client_id', ['N/A'])[0]}")
                logger.info(f"   Scope: {tokens['oauth2_parameters'].get('scope', ['N/A'])[0]}")
            ...
            if tokens.get('state_token'):
                logger.info(f"✅ State token extracted: {tokens['state_token'][:20]}...")
            
            if tokens.get('authorization_code'):
                logger.info(f"✅ Authorization code extracted: {tokens['authorization_code'][:20]}...")
            
            if tokens.get('id_token'):
                logger.info(f"✅ ID token extracted: {tokens['id_token'][:20]}...")
            
            if tokens.get('refresh_token'):
                logger.info("✅ Refresh token available - tokens can be automatically renewed")
            else:
                logger.warning("⚠️ No refresh token found - tokens will expire and require manual renewal")
    ...
    def analyze_and_extract(self) -> bool:
        ...
        if full_login_found and login_tokens.get('access_token'):
            ...
            logger.info("🎯 Authentication context found - storing OAuth2 parameters for future use...")
            self.auth_tokens = login_tokens
        ...
        if validation['can_refresh']:
            ...
            logger.info("⚠️ Tokens cannot be automatically refreshed - manual renewal required")
