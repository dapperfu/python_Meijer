...
class EnhancedMeijerHeaderSpoofer:
    ...
    def _add_dynamic_headers(
        self, headers: Dict[str, str], workflow_type: str, **kwargs
    ):
        ...
        if workflow_type in self.workflow_context:
            sequence_number = (
                self.workflow_context[workflow_type].get("sequence", 0) + 1
            )
...
if __name__ == "__main__":
    ...
    print(
        f"  Active Workflows: {', '.join(summary['session_stats']['active_workflows'])}"
    )
