...
@click.option("--store-id", "-s", type=str, help="Bind the watch to a specific store")
@click.option("--desired-price", type=click.FLOAT, help="Target price for alerts")
@click.option(
    "--only-when",
    type=click.Choice(
        ["any_drop", "sale_or_clearance", "clearance_only", "below_desired_price"]
    ),
    default="any_drop",
    help="Condition for triggering alerts",
)
@click.option("--note", type=str, help="Optional note about the watch")
def add_watch(
    identifier: str,
...
@click.argument("identifier", type=str)
@click.option(
    "--store-id", "-s", type=str, help="Remove watch from specific store only"
)
...
@click.option(
    "--all", "show_all", is_flag=True, help="Show all watches including inactive"
)
@click.option(
    "--format",
...
@click.option("--store-id", "-s", type=str, help="Filter by store ID")
def list_watches(
    active_only: bool, show_all: bool, format: str, store_id: Optional[str]
):
    """
    ...
    try:
        ...
        watches = manager.list_watches(
            active_only=active_only and not show_all, store_id=store_id
        )
...
@click.option(
    "--store-id", "-s", type=str, help="Refresh prices for specific store only"
)
@click.option("--limit", "-l", type=int, help="Maximum number of products to refresh")
@click.option(
    "--method",
    "-m",
...
def refresh_prices(store_id: Optional[str], limit: Optional[int], method: str):
    """
    ...
    try:
        ...
        # Refresh prices
        result = manager.refresh_prices(store_id=store_id, limit=limit, method=method)
...
def generate_report(since: Optional[datetime], format: str):
    ...
    try:
        ...
        if since:
            console.print(
                f"📊 [blue]Generating report since {since.strftime('%Y-%m-%d %H:%M:%S')}[/blue]"
            )
        else:
        ...
        if not alerts:
            console.print(
                "📝 [yellow]No price changes detected in the specified period[/yellow]"
            )
            return
...
@click.option(
    "--dry-run", is_flag=True, help="Show what would be emailed without sending"
)
@click.option(
    "--since",
    type=click.DateTime(formats=["%Y-%m-%d", "%Y-%m-%d %H:%M:%S"]),
...
def send_notifications(dry_run: bool, since: Optional[datetime], channel: str):
    """
...
def export_watches(file_path: Path, active_only: bool):
    ...
    try:
        ...
        console.print(
            f"✅ [green]Exported {len(watches)} watches to {file_path}[/green]"
        )
...
def setup_email_config(force: bool):
    ...
    try:
        if force:
            console.print(
                "🔄 [yellow]Running interactive configuration setup (overwriting existing)...[/yellow]"
            )
        else:
...
@click.option("--store-id", "-s", type=str, help="Use specific store for test")
@click.option(
    "--to",
    "-t",
    type=str,
...
def test_email(identifier: Optional[str], store_id: Optional[str], to: Optional[str]):
    ...
    try:
        ...
        if identifier:
            ...
            with db_session:
                ...
                if product:
                    ...
                    console.print(
                        "📝 [yellow]No products found in database, using default test data[/yellow]"
                    )
                    product_info = None

        if product_info:
            console.print(
                f"✅ [green]Using product: {product_info['product_name']} ({product_info['product_identifier']})[/green]"
            )
        ...
        message_id = email_sender.send_test_email(
            to_email=to, product_info=product_info
        )
        ...
        if message_id:
            ...
            console.print(
                "   If you don't receive it, check your spam folder and email configuration."
            )
        else:
...
@click.argument("identifier", type=str)
@click.option("--store-id", "-s", type=str, help="Get price for specific store")
@click.option(
    "--method",
    "-m",
    type=click.Choice(["cart", "shop_scan", "search", "keywords"]),
...
@click.option("--no-store", is_flag=True, help="Don't store the price in history")
def get_price(identifier: str, store_id: Optional[str], method: str, no_store: bool):
    """
    ...
    try:
        ...
        console.print(
            f"🔍 [blue]Fetching price for {identifier} using {method}...[/blue]"
        )
...
def _show_dry_run_alerts(alerts: List[Dict[str, Any]]) -> None:
    ...
    for i, alert in enumerate(alerts, 1):
        console.print(f"\n[bold]Alert {i}:[/bold]")
        console.print(
            f"  Product: {alert['product_name']} ({alert['product_identifier']})"
        )
        ...
        console.print(
            f"  Observed: {alert['observed_at'].strftime('%Y-%m-%d %H:%M:%S')}"
        )
