...
def auth_group():
    ...
    2. REGISTER: meijer auth register
       - Create a new Meijer account with interactive prompts
       - Generate configuration file: meijer auth register --config-gen
       - Use configuration file: meijer auth register --config <file>

    3. VALIDATE: meijer auth validate-phone <number> / meijer auth validate-email <email>
       - Validate phone numbers before registration
       - Check email availability for new accounts

    4. CONFIG: meijer auth config
       - View and manage authentication configuration
       - Edit headers, timing, and other parameters

    4. STATUS: meijer auth status
       - Show current authentication status and token information

    5. REFRESH: meijer auth refresh
       - Force refresh of authentication tokens
       - Test token refresh functionality

    6. LOGOUT: meijer auth logout
       - Clear stored tokens and logout

    7. IMAP: meijer auth imap
    ...
    For new accounts: meijer auth register
    """
    pass
...
@click.option(
    ...
    "-g",
    is_flag=True,
    help="Generate a TOML configuration file for registration",
)
@click.option("--config", "-c", help="Use a TOML configuration file for registration")
@click.option(
    "--output", "-o", help="Output file for generated config (default: auth.toml)"
)
def auth_register_command(config_gen: bool, config: str, output: str = None):
    """Create a new Meijer account.

    This command provides three modes:

    1. INTERACTIVE: meijer auth register
       - Prompts for all required information
       - Validates input before submission
       - Creates account immediately

    2. CONFIG GENERATION: meijer auth register --config-gen
       - Prompts for all required information
       - Saves to TOML configuration file
       - Allows editing before submission

    3. CONFIG-BASED: meijer auth register --config <file>
       - Uses existing TOML configuration file
       - Creates account from saved settings
       - No prompts required
    ...
    - Password
    - ZIP Code
    - Store ID (optional, will be detected)
    """
    if config_gen:
        _generate_registration_config(output)
    elif config:
        _register_from_config(config)
    else:
        _interactive_registration()


def _generate_registration_config(output: str = None) -> None:
    """Generate a TOML configuration file for registration."""
    if output is None:
        output = "auth.toml"

    click.echo("📝 Generating Meijer Registration Configuration")
    click.echo("=" * 50)
    click.echo("This will create a TOML file with all required registration fields.")
    click.echo(
        "You can edit the file before running: meijer auth register --config auth.toml"
    )
    click.echo()
    ...
    phone = click.prompt("📱 Phone Number (10 digits)", type=str)
    ...
    zip_code = click.prompt("🏠 ZIP Code", type=str)

    # Optional fields
    store_id = click.prompt(
        "🏪 Store ID (optional, press Enter to auto-detect)", type=str, default=""
    )
    ...
    config_data = {
        "registration": {
            ...
            "notes": "Generated by Meijer CLI - Edit this file before using",
        }
    }
    ...
    try:
        ...
        click.echo(f"1. Review and edit {output} if needed")
        click.echo(f"2. Run: meijer auth register --config {output}")
        click.echo()
        click.echo(
            "⚠️  Note: The password is stored in plain text. Consider removing it"
        )
        click.echo(
            "   from the file and entering it interactively during registration."
        )

    except ImportError:
        click.echo("❌ TOML library not available. Install with: pip install toml")
    except Exception as e:
        click.echo(f"❌ Error saving configuration: {e}")


def _register_from_config(config_file: str) -> None:
    """Register using a TOML configuration file."""
    ...
    try:
        ...
        missing_fields = [
            field for field in required_fields if not registration.get(field)
        ]
        ...
        click.echo("📋 Registration Configuration Loaded")
        click.echo("=" * 40)
        click.echo(f"👤 Name: {registration['first_name']} {registration['last_name']}")
        click.echo(f"📧 Email: {registration['email']}")
        click.echo(f"📱 Phone: {registration['phone']}")
        click.echo(f"🏠 ZIP: {registration['zip_code']}")
        if registration.get("store_id"):
            click.echo(f"🏪 Store ID: {registration['store_id']}")
        click.echo()

        # Confirm registration
        if not click.confirm("Proceed with account creation?"):
            click.echo("❌ Registration cancelled")
            return

        # Perform registration
        _perform_registration(registration)
...
def _interactive_registration() -> None:
    """Perform interactive registration with user prompts."""
    click.echo("🚀 Meijer Account Registration")
    click.echo("=" * 40)
    click.echo("Please provide the following information to create your account:")
    click.echo()
    ...
    # Confirm information
    click.echo()
    click.echo("📋 Registration Summary:")
    click.echo(f"👤 Name: {first_name} {last_name}")
    click.echo(f"📧 Email: {email}")
    click.echo(f"📱 Phone: {phone}")
    click.echo(f"🏠 ZIP: {zip_code}")
    if store_id:
        click.echo(f"🏪 Store ID: {store_id}")
    click.echo()
    ...
    # Perform registration
    _perform_registration(registration_data)


def _perform_registration(registration_data: dict) -> None:
    """Perform the actual registration process."""
    click.echo("🔄 Creating Meijer account...")
    click.echo()

    try:
        # Import registration client
        from ..registration import MeijerRegistrationClient

        # Create registration client
        client = MeijerRegistrationClient()

        # Validate phone number first
        click.echo("📱 Validating phone number...")
        phone_validation = client.validate_phone(registration_data["phone"])

        if not phone_validation.get("isValid", False):
            click.echo(
                f"❌ Phone number validation failed: {phone_validation.get('message', 'Unknown error')}"
            )
            return

        click.echo("✅ Phone number validated")

        # Create account
        click.echo("👤 Creating account...")
        account_result = client.create_account(registration_data)

        if account_result.get("success", False):
            click.echo("🎉 Account created successfully!")
            click.echo()
            click.echo("📋 Account Details:")
            click.echo(f"   Account ID: {account_result.get('accountId', 'N/A')}")
            click.echo(f"   mPerks ID: {account_result.get('mPerksId', 'N/A')}")
            click.echo()
            click.echo("💡 You can now login with:")
            click.echo("   meijer auth log --mode full")
            click.echo()
            click.echo("⚠️  Note: You may need to verify your email address before")
            click.echo("   you can access all features.")
        else:
            click.echo(
                f"❌ Account creation failed: {account_result.get('message', 'Unknown error')}"
            )

    except ImportError:
        click.echo("❌ Registration module not available")
        click.echo("💡 This feature may not be fully implemented yet")
    except Exception as e:
        click.echo(f"❌ Registration failed: {e}")
        click.echo("💡 Please check your information and try again")


@auth_group.command("validate-phone")
@click.argument("phone_number", type=str)
def auth_validate_phone_command(phone_number: str):
    """Validate a phone number for account registration.

    This command checks if a phone number is valid and available
    for use with a new Meijer account.

    Example:
        meijer auth validate-phone 5551234567
    """
    click.echo("📱 Phone Number Validation")
    click.echo("=" * 40)
    ...
    try:
        ...
        client = MeijerRegistrationClient()
        result = client.validate_phone(phone_number)

        if result.is_valid:
            click.echo("✅ Phone number is valid and available")
            click.echo(f"📱 Number: {result.formatted_number}")
            click.echo(f"🌍 Country: {result.country_code}")
            click.echo(f"💬 Textable: {getattr(result, 'is_textable', 'Unknown')}")
            click.echo(f"📱 Device Type: {getattr(result, 'device_type', 'Unknown')}")
        else:
            click.echo("❌ Phone number validation failed")
            click.echo(f"📱 Number: {phone_number}")
...
@auth_group.command("validate-email")
@click.argument("email", type=str)
def auth_validate_email_command(email: str):
    """Validate an email address for account registration.

    This command checks if an email address is valid and available
    for use with a new Meijer account.

    Example:
        meijer auth validate-email user@example.com
    """
    click.echo("📧 Email Address Validation")
    click.echo("=" * 40)
    ...
    try:
        ...
        if is_valid:
            click.echo("✅ Email address is valid and available")
            click.echo(f"📧 Address: {email}")
        else:
            click.echo("❌ Email address validation failed")
            click.echo(f"📧 Address: {email}")
            click.echo("❌ Email may be invalid or already registered")
...
@click.option(
    "--output",
    "-o",
    help="Output file for tokens (default: config directory)",
)
def auth_log_command(mode: str, log_file: Optional[str], output: str = None):
    ...
    # Set default output path to config directory if not specified
    if output is None:
        from ..auth import get_meijer_config_path

        output = get_meijer_config_path("auth.json")
        click.echo(f"📁 Using default output: {output}")
    ...
    try:
        ...
        if success:
            ...
            if mode == "quick":
                click.echo(
                    "✅ Bearer tokens extracted from API calls - ready for immediate use"
                )
            elif mode == "full":
                click.echo("✅ Full OAuth2 flow extracted - tokens can be refreshed")
            else:
                click.echo("✅ Authentication tokens extracted successfully")
...
def auth_status_command():
    ...
    This command prominently displays refresh token status and provides
    guidance on how to obtain refresh tokens for automatic token renewal.

    Use -v flag to see detailed token information and warnings.
    """
    ...
    try:
        ...
        if token_storage.has_tokens():
            ...
            if tokens:
                ...
                # Prominently display refresh token status
                if tokens.refresh_token and tokens.refresh_token.strip():
                    click.echo(f"🔄 Refresh token: {tokens.refresh_token[:30]}...")
                    click.echo("✅ Token refresh capability: ENABLED")
                else:
                    click.echo("⚠️  Refresh token: NOT AVAILABLE")
                    click.echo("❌ Token refresh capability: DISABLED")
                    click.echo(
                        "💡 Use 'meijer auth log --mode full' to get refresh tokens"
                    )
            ...
            click.echo("💡 Use 'meijer auth log' to authenticate")
...
@click.option(
    "--force", "-f", is_flag=True, help="Force refresh even if tokens are not expired"
)
def auth_refresh_command(force: bool):
    """Force refresh of authentication tokens.

    This command manually triggers a token refresh to test the refresh functionality
    and ensure tokens are working correctly.

    Use this to test your refresh token setup:
    1. meijer list show
    2. meijer auth refresh
    3. meijer list show

    This verifies that token refresh works correctly.
    """
    click.echo("🔄 Forcing Token Refresh")
    click.echo("=" * 50)
    ...
    try:
        ...
        token_storage = TokenStorage()
        if not token_storage.has_tokens():
            click.echo("❌ No tokens found")
            click.echo("💡 Use 'meijer auth log --mode full' to authenticate first")
            return

        tokens = token_storage.get_valid_tokens()
        if not tokens or not tokens.refresh_token or not tokens.refresh_token.strip():
            click.echo("❌ No refresh token available")
            click.echo("💡 Use 'meijer auth log --mode full' to get refreshable tokens")
            return

        click.echo(f"🔑 Current access token: {tokens.access_token[:30]}...")
        click.echo(f"🔄 Refresh token: {tokens.refresh_token[:30]}...")

        if not force and not tokens.is_expired(buffer_seconds=0):
            click.echo("⚠️  Tokens are not expired yet")
            click.echo("💡 Use --force to refresh anyway")
            return

        click.echo("🔄 Attempting token refresh...")

        # Force refresh using token storage
        if token_storage.refresh_tokens(tokens.refresh_token):
            click.echo("✅ Token refresh successful!")

            # Load the new tokens
            click.echo(f"🔑 New access token: {tokens.access_token[:30]}...")
            click.echo(f"⏰ New expiration: {tokens.expires_in} seconds")
            ...
            try:
                ...
                items = test_client.list.get()
                click.echo(
                    f"✅ New tokens work - Found {len(items)} shopping list items"
                )
            except Exception as e:
                click.echo(f"❌ New tokens failed: {e}")
                return

        else:
            click.echo("❌ Token refresh failed")
            click.echo(
                "💡 You may need to re-authenticate with 'meijer auth log --mode full'"
            )
            return

    except Exception as e:
        click.echo(f"❌ Error during token refresh: {e}")
        click.echo("💡 Check your authentication setup and try again")
