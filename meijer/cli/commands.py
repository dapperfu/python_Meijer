...
def auth_group():
    ...
    5. LOGOUT: meijer auth logout
       - Clear stored tokens and logout

    6. IMAP: meijer auth imap
...
@click.option(
    "--config",
    "-c",
    help="Use a TOML configuration file for registration"
)
@click.option(
    "--output",
    "-o",
    help="Output file for generated config (default: auth.toml)"
)
...
def _generate_registration_config(output: str = None) -> None:
    ...
    click.echo("You can edit the file before running: meijer auth register --config auth.toml")
    click.echo()
    ...
    # Optional fields
    store_id = click.prompt("🏪 Store ID (optional, press Enter to auto-detect)", type=str, default="")
    ...
    try:
        ...
        click.echo()
        click.echo("⚠️  Note: The password is stored in plain text. Consider removing it")
        click.echo("   from the file and entering it interactively during registration.")
...
def _register_from_config(config_file: str) -> None:
    ...
    try:
        ...
        required_fields = ["first_name", "last_name", "email", "phone", "password", "zip_code"]
        missing_fields = [field for field in required_fields if not registration.get(field)]
        
        if missing_fields:
            click.echo(f"❌ Missing required fields in config: {', '.join(missing_fields)}")
            return
...
def _perform_registration(registration_data: dict) -> None:
    ...
    try:
        ...
        if not phone_validation.get("isValid", False):
            click.echo(f"❌ Phone number validation failed: {phone_validation.get('message', 'Unknown error')}")
            return
        ...
        if account_result.get("success", False):
            ...
            click.echo(f"❌ Account creation failed: {account_result.get('message', 'Unknown error')}")
...
def auth_log_command(mode: str, log_file: Optional[str], output: str = None):
    ...
    try:
        ...
        if success:
            ...
            if mode == "quick":
                click.echo("✅ Bearer tokens extracted from API calls - ready for immediate use")
            elif mode == "full":
        ...
        if token_storage.has_tokens():
            ...
            if tokens:
                ...
                if tokens.refresh_token and tokens.refresh_token.strip():
                    ...
                    click.echo("💡 Use 'meijer auth log --mode full' to get refresh tokens")
