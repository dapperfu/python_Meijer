...
class ComprehensiveShopScanAnalyzer:
    ...
    def analyze_log_file(self, log_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"   🏪 Found {len(results['shop_endpoints'])} shop-related endpoints"
            )
            print(
                f"   📱 Found {len(results['scan_endpoints'])} scan-related endpoints"
            )
            ...
            try:
                ...
                if self._is_relevant_json(json_data):
                    ...
                    endpoint_match = re.search(
                        r"api\.meijer\.com([^?\s]+)", context_before
                    )
                    if endpoint_match:
...
def main():
    ...
    for log_name, results in all_results.items():
        ...
        if "file_size_mb" in results:
            comprehensive_report["analysis_summary"]["total_file_size_mb"] += results[
                "file_size_mb"
            ]
    ...
    print(
        f"📊 Analyzed {comprehensive_report['analysis_summary']['total_file_size_mb']:.1f} MB of log data"
    )
