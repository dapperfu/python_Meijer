...
def detect_authentication_flow(flows: List[HTTPFlow]) -> Dict[str, Any]:
    ...
    for flow in flows:
            ...
            flow_analysis["flow_sequence"].append(
                {
                    ...
                    "timestamp": timestamp,
                    "url": url,
                    "cookies_set": dict(flow.response.cookies)
                    if flow.response and flow.response.cookies
                    else {},
                }
            )
            ...
            flow_analysis["flow_sequence"].append(
                {
                    ...
                    "timestamp": timestamp,
                    "url": url,
                    "state_token": parse_qs(urlparse(url).query).get(
                        "stateToken", [None]
                    )[0],
                }
            )
        ...
        # Track OAuth2 token endpoints
            ...
            flow_analysis["flow_sequence"].append(
                {"step": "token_exchange", "timestamp": timestamp, "url": url}
            )
            ...
            api_calls.append(
                {
                    ...
                    in [h.lower() for h in flow.request.headers.keys()],
                    "cookies": dict(flow.request.cookies)
                    if flow.request.cookies
                    else {},
                }
    ...
    if device_fingerprint_endpoints and login_redirect_endpoints:
        ...
        flow_analysis["key_endpoints"].extend(["device_fingerprint", "login_redirect"])
...
def analyze_jwt_token(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        header = json.loads(
            base64.urlsafe_b64decode(header_b64 + "=" * (-len(header_b64) % 4)).decode(
                "utf-8"
            )
        )
        payload = json.loads(
        ...
        if (
            payload.get("iss") == "https://id.meijer.com/oauth2/default"
            and payload.get("aud") == "api://default"
            and "offline_access" in payload.get("scp", [])
        ):
...
def main():
    ...
    if oauth2_responses:
        ...
        for i, response in enumerate(oauth2_responses[:5]):  # Show first 5
            ...
            # Show JWT analysis
            if "jwt_analysis" in response and response["jwt_analysis"].get(
                "is_authentic"
            ):
    ...
    if bearer_requests:
        ...
        for i, request in enumerate(bearer_requests[:5]):  # Show first 5
            ...
            # Show JWT analysis
            if "jwt_analysis" in request and request["jwt_analysis"].get(
                "is_authentic"
            ):
        ...
        if best_tokens:
            ...
            if best_tokens.get("source") == "oauth2_token_exchange":
                ...
                if "jwt_analysis" in best_tokens and best_tokens["jwt_analysis"].get(
                    "is_authentic"
                ):
    ...
    if flow_analysis["flow_type"] == "full_oauth2_login":
        ...
        print(
            "   💡 Consider running 'meijer auth full-login' for complete authentication"
        )
