...
def analyze_complex_promotions(flows: List[HTTPFlow]) -> Dict[str, Any]:
    ...
    for flow in flows:
        ...
        if "complexpromos" in flow.request.path:
            ...
            for header, value in flow.request.headers.items():
                ...
                if any(
                    auth_key in header_lower
                    for auth_key in ["authorization", "bearer", "token", "x-", "cookie"]
                ):
    ...
    return {
        ...
        "successful_calls": len(
            [
                f
                for f in complex_promo_flows
                if f["status_code"] and f["status_code"] < 400
            ]
        ),
        "failed_calls": len(
            [
...
def main():
    ...
    if analysis["flows"]:
        ...
        successful_flows = [
            f for f in analysis["flows"] if f["status_code"] and f["status_code"] < 400
        ]
        failed_flows = [
            f for f in analysis["flows"] if f["status_code"] and f["status_code"] >= 400
        ]
