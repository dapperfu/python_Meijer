...
class FlaskApiServerUpdater:
    ...
    def load_existing_app(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded existing Flask app with {len(self.existing_routes)} routes"
            )
            return True
    ...
    def _extract_existing_routes(self) -> None:
        ...
        self.logger.info(
            f"Found {len(self.existing_routes)} existing routes and {len(self.existing_functions)} functions"
        )
    ...
    def load_endpoints_data(self) -> bool:
        ...
        if not self.endpoints_data_path.exists():
            self.logger.error(
                f"Endpoints data file not found: {self.endpoints_data_path}"
            )
            return False
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.endpoints_data.get('endpoints', {}))} endpoints, {len(self.new_endpoints)} are new"
            )
            return True
    ...
    def generate_new_routes(self) -> str:
        ...
        for path in sorted(self.new_endpoints):
            ...
            methods = (
                list(endpoint_info.get("methods", set()))
                if endpoint_info.get("methods")
                else ["GET"]
            )
            if not methods:
                methods = ["GET"]
...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python update_flask_api_server.py <flask_app.py> <endpoints_data.json> [output_dir]"
        )
        sys.exit(1)
