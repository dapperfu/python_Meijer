...
class SimpleCouponAnalyzer:
    ...
    def _process_line(self, line: str, line_num: int) -> None:
        ...
        if any(
            re.search(pattern, line, re.IGNORECASE) for pattern in self.coupon_patterns
        ):
    ...
    def _categorize_endpoint(self, url: str, line: str, line_num: int) -> None:
        ...
        endpoint_data = {"url": url, "line": line_num, "full_line": line}
            ...
            if field in data and isinstance(data[field], list):
                for item in data[field]:
                    if isinstance(item, dict):
                        reward_info = {"line": line_num, "data": item, "field": field}
