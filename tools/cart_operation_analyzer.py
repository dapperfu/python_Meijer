...
class CartOperationAnalyzer:
    ...
    def _generate_report(self) -> None:
        ...
        print(f"\n📋 SUMMARY")
        print("=" * 40)
        print(f"Total operations found: {len(self.cart_operations) + len(self.rewards_operations) + len(self.milk_operations) + len(self.bread_operations) + len(self.quantity_changes)}")
        
        if not any([self.cart_operations, self.rewards_operations, self.milk_operations, self.bread_operations, self.quantity_changes]):
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("Example: python3 cart_operation_analyzer.py meijer_mitm_20250818_130318.log")
        sys.exit(1)
