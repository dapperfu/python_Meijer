...
def find_pattern_matches(content: bytes, pattern: bytes) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        matches.append({"position": pos, "pattern": pattern.decode("utf-8")})
...
def extract_device_fingerprint_sequence(
    content: bytes, position: int
...
def save_analysis_results(
    analysis: Dict[str, Any], output_file: str = "auth_sequence_analysis.json"
):
