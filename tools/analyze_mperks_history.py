...
class MPerksHistoryAnalyzer:
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        if "mperks" in flow.request.pretty_url.lower():
            ...
            if any(keyword in flow.request.pretty_url.lower() for keyword in 
                   ["history", "transaction", "activity", "points", "earn", "redeem"]):
                ...
                if flow_info["flow_id"] == flow.id:
                    ...
                    try:
                        if "application/json" in flow.response.headers.get("content-type", ""):
                            flow_info["response_data"] = json.loads(flow.response.content.decode('utf-8'))
                        else:
                            flow_info["response_data"] = flow.response.content.decode('utf-8', errors='ignore')[:500]
    ...
    def done(self):
        ...
        ctx.log.info(f"Analysis complete. Found {len(self.mperks_history_flows)} mPerks history flows.")
