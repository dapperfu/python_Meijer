...
def create_login_flow_summary(log_path: Path) -> None:
    ...
    try:
        ...
        if "refresh_token" in tokens:
            print(
                "✅ REFRESH TOKEN FOUND - This is the key token for re-authentication!"
            )
            ...
            print(
                "❌ REFRESH TOKEN NOT FOUND - You'll need to capture login data each time"
            )

        if "serversidesignal" in tokens or any(
            "serversidesignal" in k for k in tokens.keys()
        ):
