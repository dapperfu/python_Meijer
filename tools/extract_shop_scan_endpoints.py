...
def load_api_endpoints_analysis(
    file_path: str = "api_endpoints_analysis.json",
) -> Dict[str, Any]:
        ...
        for endpoint, count in endpoints.items():
            if is_shop_scan_endpoint(endpoint):
                ...
                endpoint_info = {
                    ...
                    "count": count,
                    "log_file": log_file,
                    "method": call_details.get("method", "UNKNOWN")
                    if call_details
                    else "UNKNOWN",
    ...
    return {
        "summary": {
            ...
            "unique_endpoints": len(
                set(endpoint["endpoint"] for endpoint in all_shop_scan_endpoints)
            ),
        },
...
def categorize_shop_scan_endpoints(
    ...
    for endpoint_info in endpoints:
        ...
        if any(keyword in endpoint for keyword in ["barcode", "upc", "scan", "lookup"]):
            ...
            keyword in endpoint for keyword in ["fulfillment", "delivery", "pickup"]
        ):
...
def display_shop_scan_summary(shop_scan_data: Dict[str, Any]):
    ...
    for category, category_endpoints in categories.items():
        if category_endpoints:
            print(
                f"  {category.replace('_', ' ').title()}: {len(category_endpoints)} endpoints"
            )
