...
def extract_http_requests(log_file_path: str):
    ...
    try:
        ...
        http_request_pattern = (
            rb"(GET|POST|PUT|DELETE|PATCH)\s+([^\s\r\n]+)\s+HTTP/[0-9.]+"
        )
        ...
        # OAuth2 authorize
        oauth2_requests = find_requests_by_url_pattern(
            content, rb"oauth2/default/v1/authorize"
        )
        if oauth2_requests:
        ...
        # Device fingerprint
        device_fp_requests = find_requests_by_url_pattern(
            content, rb"auth/services/devicefingerprint"
        )
        ...
        # Device nonce
        device_nonce_requests = find_requests_by_url_pattern(
            content, rb"api/v1/internal/device/nonce"
        )
        ...
        # Token exchange
        token_requests = find_requests_by_url_pattern(
            content, rb"oauth2/default/v1/token"
        )
        if token_requests:
...
def find_requests_by_url_pattern(
    content: bytes, url_pattern: bytes
) -> List[Dict[str, Any]]:
...
def extract_context_at_position(
    content: bytes, position: int, context_size: int
) -> str:
...
def parse_http_request(
    context_text: str, pattern_offset: int
