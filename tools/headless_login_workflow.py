    ...
    def step4_web_login_identify(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"❌ Identify failed: {response.status_code} - {response.text}"
                )
                return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if challenge_data.get("type") == "MFA":
                    logger.info(
                        "📱 MFA required - this workflow doesn't handle MFA yet"
                    )
                    return False
                ...
                logger.error(
                    f"❌ Password challenge failed: {response.status_code} - {response.text}"
                )
                return False
    ...
    def step6_oauth_keys(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ OAuth keys received: {len(keys_data.get('keys', []))} keys"
                )
                return True
            else:
    ...
    def step7_token_exchange(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"🔑 Access token: {self.access_token[:20] if self.access_token else 'None'}..."
                )
                logger.info(
                    f"🆔 ID token: {self.id_token[:20] if self.id_token else 'None'}..."
                )
                return True
            else:
                logger.error(
