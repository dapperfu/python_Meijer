 ...
 * - Dependencies: meijer package, logging, pathlib, json
 */
...
from meijer.price_monitor import PriceMonitor
...
def run_all_monitors(price_monitor: PriceMonitor) -> Dict[str, Any]:
    ...
    for monitor_id, config in monitors.items():
        ...
        try:
            ...
            logging.info(
                f"Monitor '{config['name']}' completed: {monitor_results['products_found']} products found"
            )
...
def check_clearance_deals(
    price_monitor: PriceMonitor, min_discount: float = 20.0
) -> List[Dict[str, Any]]:
    """
...
def main():
    ...
    try:
        ...
        logging.info("Initializing Meijer client...")
        meijer = Meijer()

        # Check authentication
        if not meijer.is_authenticated():
            logging.error(
                "Not authenticated. Please ensure valid credentials are available."
            )
            sys.exit(1)
        ...
        price_monitor = PriceMonitor(meijer, data_dir)
