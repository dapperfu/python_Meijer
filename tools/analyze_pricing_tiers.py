        ...
        pricing_info["cart_totals"].append(
            {
                ...
                "savings": savings_val,
                "savings_percentage": (savings_val / cart_was_val * 100)
                if cart_was_val > 0
                else 0,
            }
        )
        ...
        for upc in target_upcs:
            ...
            if upc_indices:
                ...
                if pricing_info["cart_totals"]:
                    ...
                    print(
                        f"{'Step':<4} {'Cart Now':<10} {'Cart Was':<10} {'Savings':<10} {'Savings %':<10}"
                    )
                    print("-" * 60)

                    for i, cart in enumerate(pricing_info["cart_totals"]):
                        print(
                            f"{i + 1:<4} ${cart['cart_now']:<9.2f} ${cart['cart_was']:<9.2f} ${cart['savings']:<9.2f} {cart['savings_percentage']:<9.1f}%"
                        )
                ...
                if pricing_info["quantity_patterns"]:
                    ...
                    for pattern in pricing_info["quantity_patterns"]:
                        print(
                            f"  Quantity: {pattern['quantity']} - {pattern['pricing_text']}"
                        )
                ...
                if pricing_info["cart_totals"]:
                    ...
                    cart_totals = [
                        cart["cart_now"] for cart in pricing_info["cart_totals"]
                    ]
                            ...
                            if i % 2 == 1:  # Odd steps (1, 3, 5...)
                                if i < len(cart_totals) - 1:
                                    ...
                                    if step2_increase < step1_increase:
                                        print(
                                            f"  Steps {i}-{i + 1}: Potential BOGO - second item cheaper"
                                        )
                                        print(f"    First item: ${step1_increase:.2f}")
                                        print(f"    Second item: ${step2_increase:.2f}")
                                        print(
                                            f"    BOGO savings: ${step1_increase - step2_increase:.2f}"
                                        )
                        ...
                        for i in range(3, len(cart_totals)):
                            if i % 4 == 3:  # Every 4th step
                                if i < len(cart_totals) - 1:
                                    ...
                                    if (
                                        step4_increase < avg_previous * 0.8
                                    ):  # 20% or more discount
                                        print(
                                            f"  Steps {i}-{i + 1}: Potential tiered pricing - 4th item discounted"
                                        )
                                        print(
                                            f"    Average previous: ${avg_previous:.2f}"
                                        )
                                        print(f"    4th item: ${step4_increase:.2f}")
                                        print(
                                            f"    Discount: ${avg_previous - step4_increase:.2f}"
                                        )
...
def generate_pricing_summary(results: Dict[str, Dict[str, Any]]) -> None:
    ...
    for upc, pricing_info in results.items():
        if pricing_info.get("cart_totals"):
            ...
            overall_savings_pct = (
                (total_savings / total_was * 100) if total_was > 0 else 0
            )
            ...
            if len(cart_totals) > 1:
                ...
                for i in range(1, len(cart_totals)):
                    ...
                    if i == 1:
                        ...
                        avg_previous = sum(
                            cart_totals[j]["cart_now"] - cart_totals[j - 1]["cart_now"]
                            for j in range(1, i)
                        ) / (i - 1)
