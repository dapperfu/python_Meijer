...
def search_api_endpoints():
    ...
    account_keywords = [
        'account', 'profile', 'user', 'receipt', 'order', 'savings',
        'loyalty', 'digital', 'mperks', 'preferences', 'settings'
    ]
    
    found_endpoints = set()
    
    try:
        with open('logs/meijer_mitm_20250822_2130.log', 'r', encoding='utf-8', errors='ignore') as f:
            for line_num, line in enumerate(f, 1):
                for pattern in api_patterns:
                    matches = re.findall(pattern, line, re.IGNORECASE)
                    for match in matches:
                        # Check if it contains account-related keywords
                        if any(keyword in match.lower() for keyword in account_keywords):
                            # Clean up the URL
                            clean_url = match.split('?')[0].split(' ')[0]
                            found_endpoints.add(clean_url)
                            
                            if len(found_endpoints) % 5 == 0:
                                print(f"Found {len(found_endpoints)} API endpoints so far...")
