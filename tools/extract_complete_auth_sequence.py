...
def extract_complete_auth_sequence():
    ...
    try:
        ...
        for match in post_requests:
            ...
            auth_events.append(
                {"type": event_type, "position": pos, "context": context}
            )
        ...
        auth_events.append(
            {
                ...
                "context": content_str[
                    max(0, challenge_pos - 1000) : challenge_pos + 1000
                ],
            }
        )
            ...
            for match in header_matches:
                if len(match) == 5:
                    (
                        ...
                        header_value_length,
                        header_value,
                    ) = match[0], match[1], match[3], match[4]
                    if len(header_name) == int(header_name_length) and len(
                        header_value
