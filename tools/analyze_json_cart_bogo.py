...
def extract_upc_from_json_content(json_content: Any) -> Optional[str]:
    ...
    if isinstance(json_content, dict):
        ...
        upc_fields = [
            'upc', 'UPC', 'barcode', 'Barcode', 'barcodeData', 'BarcodeData',
            'itemSku', 'ItemSku', 'sku', 'SKU', 'productId', 'ProductId'
        ]
...
def analyze_cart_progression_for_bogo(cart_flows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
def analyze_upc_data(upc_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    shop_scan_flows = [flow for flow in upc_data if flow.get('flow_type') == 'shop_n_scan']
    
    # Sort cart flows by occurrence
    cart_flows.sort(key=lambda x: x.get('occurrence', 0))
    
    # Analyze cart progression
    cart_progression = analyze_cart_progression_for_bogo(cart_flows) if cart_flows else []
...
def search_for_specific_upc(data: Dict[str, List[Dict[str, Any]]], target_upc: str) -> Optional[Dict[str, Any]]:
    """
...
def main():
    ...
    print(f"JSON Cart BOGO Analysis")
            ...
            if upc_analysis:
                ...
                if upc_analysis['cart_progression']:
                    ...
                    for step in upc_analysis['cart_progression']:
                        ...
                        discount_info = f"{step['discount_percent']:.1f}%" if step['discount_percent'] > 0 else "0%"
                        print(f"{step['step']:<4} ${step['cart_now_total']:<11.2f} ${step['items_added']:<11.2f} ${step['price_per_item']:<11.2f} {bogo_info:<15} {discount_info:<10}")
            ...
            for upc, upc_data in data.items():
                ...
                if upc_analysis['bogo_steps']:
                    bogo_upcs.append({
                        ...
                        'bogo_types': list(set([step['bogo_type'] for step in upc_analysis['bogo_steps']]))
                    })
            
            if bogo_upcs:
                print(f"Found {len(bogo_upcs)} UPCs with BOGO patterns:")
                for bogo_upc in bogo_upcs:
                    print(f"  UPC {bogo_upc['upc']}: {bogo_upc['bogo_steps']} BOGO steps - {', '.join(bogo_upc['bogo_types'])}")
            else:
