...
def print_top_endpoints_table(data: Dict[str, Any], top_n: int = 10) -> None:
    ...
    print("=" * 120)
    print(
        f"{'Rank':<4} {'Hits':<6} {'Priority':<8} {'Category':<12} {'Method':<6} {'Endpoint':<70}"
    )
    print("-" * 120)
    ...
    print("-" * 120)
    print(
        f"📊 Total missing endpoints: {data.get('summary', {}).get('total_missing', 0)}"
    )
    print(
        f"🎯 High priority (score > 50): {len([ep for ep in data.get('implementation_priority', []) if ep.get('priority_score', 0) > 50])}"
    )
    print("=" * 120)
