...
def extract_meijer_flows(flows: List[HTTPFlow]) -> List[Dict[str, Any]]:
    ...
    for flow in flows:
        ...
        if any(domain in host for domain in ['meijer.com', 'id.meijer.com', 'api.meijer.com']):
            flow_data = {
            ...
            if flow.response and flow.response.content:
                try:
                    ...
                    if "application/json" in content_type:
                        ...
                        flow_data["response_body"] = flow.request.content.decode('utf-8', errors='ignore')
                except:
                    flow_data["response_body"] = flow.response.content.decode('utf-8', errors='ignore')
...
def analyze_auth_patterns(flow_data: Dict[str, Any]) -> List[str]:
    ...
    if "ocp-apim-subscription-key" in req_headers:
        patterns.append(f"API subscription key: {req_headers['ocp-apim-subscription-key'][:20]}...")
    ...
    if "authorization" in resp_headers:
        patterns.append(f"Authorization in response: {resp_headers['authorization'][:50]}...")
    ...
    if isinstance(response_body, dict):
        ...
        for key, value in response_body.items():
            if isinstance(value, dict):
                nested_token = value.get("access_token") or value.get("token") or value.get("bearer_token")
                if nested_token and isinstance(nested_token, str) and len(nested_token) > 20:
                    tokens.append({
                        "type": "nested_token",
                        "value": nested_token,
...
def find_authentication_flows(flows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for flow in flows:
        ...
        if any(keyword in path for keyword in ['login', 'auth', 'oauth', 'token', 'signin', 'challenge']):
            auth_flows.append({
...
def main():
    ...
    for i, token in enumerate(all_tokens[:10]):  # Show first 10
        print(f"   {i+1}. {token['type']} token ({token['location']}): {token['value'][:30]}...")
