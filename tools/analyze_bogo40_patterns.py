...
def analyze_bogo40_patterns(content: str) -> Dict[str, Any]:
    ...
    bogo50_patterns = [r"bogo.*?50%", r"buy.*?one.*?get.*?50%", r"50%.*?off.*?second"]
    ...
    quantity_patterns = [r"\$\d+/\d+", r"buy\s*\d+\s*for\s*\$\d+", r"\d+\s*for\s*\$\d+"]
...
def main():
    ...
    try:
        ...
        if cart_progression:
            ...
            print(
                f"{'Step':<4} {'Cart Total':<12} {'Items Added':<12} {'Price/Item':<12} {'BOGO':<15}"
            )
            print("-" * 70)
            ...
            for step in cart_progression:
                ...
                print(
                    f"{step['step']:<4} ${step['cart_now']:<11.2f} ${step['items_added']:<11.2f} ${step['price_per_item']:<11.2f} {bogo_info:<15}"
                )
            ...
            if bogo_steps:
                ...
                for step in bogo_steps:
                    ...
                    print(
                        f"    Previous price per item: ${cart_progression[step['step'] - 2]['price_per_item']:.2f}"
                    )
