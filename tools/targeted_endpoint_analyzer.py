...
class TargetedEndpointAnalyzer:
    ...
    def __init__(self):
        ...
        self.related_endpoints = defaultdict(
            lambda: {
                "count": 0,
                "methods": set(),
                "hosts": set(),
    ...
    def _analyze_flow_for_targets(self, flow: HTTPFlow, source_file: str):
        ...
        try:
            ...
            if self._is_target_endpoint(path):
                self._process_target_endpoint(
                    flow, method, path, host, url, source_file
                )

            # Check for related endpoints (tax, address, oms, etc.)
            if self._is_related_endpoint(path):
                self._process_related_endpoint(
                    flow, method, path, host, url, source_file
                )
    ...
    def _is_target_endpoint(self, path: str) -> bool:
        ...
        target_patterns = [r"/digital/tax/v1", r"/oms/v2/address-qualification"]
    ...
    def _process_target_endpoint(
        ...
        if (
            request_body
            and len(self.target_endpoints[target_key]["sample_requests"]) < 3
        ):
            self.target_endpoints[target_key]["sample_requests"].append(request_body)

        if (
            response_body
            and len(self.target_endpoints[target_key]["sample_responses"]) < 3
        ):
            self.target_endpoints[target_key]["sample_responses"].append(response_body)

    def _process_related_endpoint(
        self,
        flow: HTTPFlow,
        method: str,
        path: str,
    ...
    def generate_target_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "related_endpoints": {},
            "summary": {
                "digital_tax_v1_found": len(
                    self.target_endpoints["digital/tax/v1"]["endpoints"]
                ),
                "oms_address_qualification_found": len(
                    self.target_endpoints["oms/v2/address-qualification"]["endpoints"]
                ),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.target_endpoints["digital/tax/v1"]["endpoints"]:
            recommendations.append(
                "Implement digital/tax/v1 API client for tax calculations"
            )
            recommendations.append(
                "Add tax calculation methods based on discovered endpoints"
            )

        # OMS Address Qualification recommendations
        if self.target_endpoints["oms/v2/address-qualification"]["endpoints"]:
            recommendations.append(
                "Implement oms/v2/address-qualification API client for shipping validation"
            )
        ...
        if self.related_endpoints:
            recommendations.append(
                f"Found {len(self.related_endpoints)} related endpoints - consider implementing tax and address services"
            )

        # General recommendations
        if any(self.target_endpoints.values()):
            recommendations.append(
                "Create dedicated modules for tax and address qualification services"
            )
            recommendations.append(
                "Integrate with existing Meijer client for seamless API access"
            )
...
def main():
    ...
    print(
        f"  OMS Address Qualification endpoints: {report['summary']['oms_address_qualification_found']}"
    )
    ...
    if report["related_endpoints"]:
        ...
        for path, data in list(report["related_endpoints"].items())[
            :10
        ]:  # Show first 10
