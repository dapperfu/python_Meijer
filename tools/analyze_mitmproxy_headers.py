#!/usr/bin/env python3
"""
Analyze Headers from Mitmproxy Logs

This tool specifically handles the mitmproxy binary log format to extract
actual request headers and analyze authentication flows.
"""
...
class MitmproxyHeaderAnalyzer:
    """Analyze headers from mitmproxy binary logs."""
    ...
    def analyze_log(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extract flows using mitmproxy format
    ...
    def _parse_request(self, data: str) -> Dict[str, Any]:
        ...
        # Extract headers - look for the headers section
    ...
    def _extract_headers_from_data(self, data: str) -> Dict[str, str]:
        ...
        # Look for the headers section: 7:headers;...
        headers_match = re.search(r"7:headers;([^}]+)", data)
        if not headers_match:
            return headers

        headers_data = headers_match.group(1)

        # Parse header format: key_length:key,value_length:value
        # Pattern: 29:6:accept,16:application/json
        header_pairs = re.findall(r"(\d+):([^,]+),(\d+):([^,]+)", headers_data)

        for key_len_str, key_part, value_len_str, value_part in header_pairs:
            try:
...
def main():
    ...
    # Analyze the log
    analyzer = MitmproxyHeaderAnalyzer(latest_log)
    ...
    output_file = f"mitmproxy_headers_analysis_{int(time.time())}.json"
