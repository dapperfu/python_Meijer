    ...
    for endpoint in known_endpoints:
        ...
        while True:
            ...
            try:
                ...
                status = (
                    f"{status_match.group(1)}: {status_match.group(2)}"
                    if status_match
                    else "UNKNOWN"
                )
                ...
                results["cart_items"].append(
                    {
                        ...
                        "scanned_upc": scanned_upc_match.group(1)
                        if scanned_upc_match
                        else None,
        ...
        while True:
            ...
            try:
                ...
                endpoint_match = re.search(
                    r"(/retail/shopandscan/[^\s]+)", context_text
                )
                ...
                transaction_id_match = re.search(
                    r'"transactionId":"([^"]+)"', context_text
                )
                transaction_id = (
                    transaction_id_match.group(1) if transaction_id_match else "UNKNOWN"
                )
...
def analyze_python_implementation_gaps(operations_found: Dict[str, Any]) -> List[str]:
    ...
    if "/dgtlmma/accounts/isShopAndScanEnabled" in operations_found.get(
        "endpoints", {}
    ):
        ...
        recommendations["missing_methods"].append(
            {
                ...
                "example": analysis["operations"]["UPDATE_QUANTITY"]["examples"][0]
                if analysis["operations"]["UPDATE_QUANTITY"]["examples"]
                else None,
            }
        )
        ...
        recommendations["missing_methods"].append(
            {
                ...
                "example": analysis["operations"]["END_TRANSACTION"]["examples"][0]
                if analysis["operations"]["END_TRANSACTION"]["examples"]
                else None,
            }
        )
            ...
            for item in items:
                if isinstance(item, dict):
                    if "method" in item:
                        ...
                        print(
                            f"  - {item['operation']}: {item['current_implementation']}"
                        )
                else:
                    print(f"  - {item}")
