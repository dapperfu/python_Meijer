            ...
            with open(self.log_file_path, "rb") as f:
                ...
                for flow in reader.stream():
                    ...
                    if flow_count % 1000 == 0:
                        self.logger.info(
                            f"Processed {flow_count} flows, found {api_meijer_count} api.meijer.com calls"
                        )

                self.logger.info(
                    f"Completed: {flow_count} total flows, {api_meijer_count} api.meijer.com calls"
                )
                return True
        ...
        if len(endpoint_info["sample_responses"]) < 3:
            try:
                ...
                sample_response = {
                    ...
                    in flow.response.headers.get("content-type", ""),
                }
    ...
    def categorize_endpoints(self) -> Dict[str, List[str]]:
        ...
        for path in self.unique_paths:
            ...
            if any(x in path_lower for x in ["/auth", "/oauth", "/login", "/token"]):
                ...
                x in path_lower for x in ["/cart", "/basket", "/shopping", "/checkout"]
            ):
                categories["Cart & Shopping"].append(path)
            elif any(
                x in path_lower for x in ["/product", "/search", "/upc", "/barcode"]
            ):
                categories["Products & Search"].append(path)
            elif any(
                x in path_lower for x in ["/mperks", "/loyalty", "/rewards", "/points"]
            ):
                ...
                x in path_lower
                for x in ["/fulfillment", "/reservation", "/pickup", "/delivery"]
            ):
                categories["Fulfillment"].append(path)
            elif any(
                x in path_lower for x in ["/user", "/profile", "/customer", "/account"]
            ):
