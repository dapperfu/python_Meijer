            ...
            results["pdf417_barcodes"].append(
                {
                    ...
                    "context": text_content[
                        max(0, match.start() - 100) : min(
                            len(text_content), match.end() + 100
                        )
                    ].strip(),
                }
            )
...
def main():
    ...
    print(
        f"Checkout flows found: {results['analysis_summary']['total_checkout_flows']}"
    )
    print(
        f"Cart summaries found: {results['analysis_summary']['total_cart_summaries']}"
    )
    print(
        f"PDF417 barcodes found: {results['analysis_summary']['total_pdf417_barcodes']}"
    )
    print(
        f"Checkout endpoints found: {results['analysis_summary']['total_checkout_endpoints']}"
    )
