...
def extract_auth_flows(log_file: Path) -> List[Dict[str, Any]]:
    ...
    try:
                ...
                if line and line.startswith("{"):
                    try:
                        ...
                        if total_flows % 1000 == 0:
                            print(
                                f"   Processed {total_flows} flows, found {len(auth_flows)} auth flows"
                            )
        ...
        print(
            f"✅ Extracted {len(auth_flows)} authentication flows from {total_flows} total flows"
        )
        return auth_flows
...
def is_auth_flow(flow: Dict[str, Any]) -> bool:
    ...
    if flow.get("method") == "POST" and any(
...
def find_successful_login_sequence(
    ...
    for i, timestamp in enumerate(sorted_times):
        ...
        if all(
            any(indicator in path for path in paths)
...
def analyze_successful_sequence(sequence: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for i, flow in enumerate(sequence):
        ...
        analysis["headers_evolution"].append(
            {"step": i + 1, "headers": flow.get("headers", {})}
        )

        # Track cookies evolution
        analysis["cookies_evolution"].append(
            {"step": i + 1, "cookies": flow.get("cookies", {})}
        )
    ...
    print(
        f"   ⏱️  Total duration: {analysis['timing'][-1]['time_diff'] if analysis['timing'] else 'unknown'} seconds"
    )
...
"""
    ...
    for cookie_name, count in cookie_counts.most_common():
        if count > 1:
            template += (
                f"    '{cookie_name}': 'REQUIRED',  # Appears in {count} steps\n"
            )
...
def save_analysis_results(
    analysis: Dict[str, Any], output_file: str = "auth_flow_analysis.json"
):
