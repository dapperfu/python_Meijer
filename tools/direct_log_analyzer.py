...
class DirectLogAnalyzer:
            ...
            try:
                ...
                if hasattr(flow, 'request') and hasattr(flow.request, 'url'):
                    ...
                    if self._is_shop_scan_endpoint(url):
                        ...
                        if any(keyword in url.lower() for keyword in ['scan', 'barcode', 'upc', 'qr', 'camera']):
    ...
    def _is_shop_scan_endpoint(self, url: str) -> bool:
        ...
        url_lower = url.lower()
        shop_keywords = ['shop', 'store', 'cart', 'item', 'product', 'barcode', 'upc', 'fulfillment', 'reservation']
...
def main():
    ...
    for log_name, results in all_results.items():
        ...
        if 'status_codes' in results:
            comprehensive_report['analysis_summary']['total_status_codes'].update(results['status_codes'])
        if 'total_flows' in results:
            comprehensive_report['analysis_summary']['total_flows_analyzed'] += results['total_flows']
    ...
    print(f"📊 Analyzed {comprehensive_report['analysis_summary']['total_flows_analyzed']} total flows")
