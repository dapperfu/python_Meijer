...
def test_shopping_list_access():
    ...
    try:
        ...
        print(f"  Access Token: {tokens.get('access_token', 'None')[:20] if tokens.get('access_token') else 'None'}...")
        print(f"  ID Token: {tokens.get('id_token', 'None')[:20] if tokens.get('id_token') else 'None'}...")
        print()
