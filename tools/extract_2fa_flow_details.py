...
def extract_2fa_flow_details():
    ...
    try:
        ...
        if username_matches:
            ...
            username_context = content_str[max(0, username_pos - 500):username_pos + 1000]
        ...
        if password_matches:
            ...
            password_context = content_str[max(0, password_pos - 500):password_pos + 1000]
            auth_sequence.append({
        ...
        verification_matches = list(re.finditer(verification_pattern, post_challenge_content))
        
        if verification_matches:
            verification_pos = challenge_pos + verification_matches[0].start()
            verification_context = content_str[verification_pos - 500:verification_pos + 1000]
            auth_sequence.append({
