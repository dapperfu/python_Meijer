...
def find_authentication_flows(flows):
    ...
    for i, flow in enumerate(flows):
        ...
        if (
            flow.request.pretty_host == "id.meijer.com"
            and flow.request.path.startswith("/oauth2/default/v1/authorize")
            and "login_hint" in flow.request.pretty_url
        ...
        # Look for logout events
            ...
            flow.request.pretty_host == "id.meijer.com"
            and flow.request.path == "/oauth2/default/v1/logout"
            and flow.response.status_code == 200
        ):
            print(f"🎯 Found successful logout event at flow {i}")
            auth_flows.append({"type": "logout", "flow_index": i, "flow": flow})

        # Look for device nonce requests
        elif (
            flow.request.pretty_host == "id.meijer.com"
            and flow.request.path == "/api/v1/internal/device/nonce"
            and flow.request.method == "POST"
            and flow.response.status_code == 200
        ):
            print(f"🎯 Found device nonce request at flow {i}")
            auth_flows.append({"type": "device_nonce", "flow_index": i, "flow": flow})

        # Look for challenge answer (password submission)
        elif (
            flow.request.pretty_host == "id.meijer.com"
            and flow.request.path == "/idp/idx/challenge/answer"
            and flow.request.method == "POST"
            and flow.response.status_code == 200
        ):
        ...
        # Look for OAuth keys request
            ...
            flow.request.pretty_host == "id.meijer.com"
            and flow.request.path.startswith("/oauth2/default/v1/keys")
            and flow.response.status_code == 200
        ):
            print(f"🎯 Found OAuth keys request at flow {i}")
            auth_flows.append({"type": "oauth_keys", "flow_index": i, "flow": flow})

        # Look for token exchange
        elif (
            flow.request.pretty_host == "id.meijer.com"
            and flow.request.path == "/oauth2/default/v1/token"
            and flow.request.method == "POST"
            and flow.response.status_code == 200
        ):
            print(f"🎯 Found token exchange at flow {i}")
            auth_flows.append({"type": "token_exchange", "flow_index": i, "flow": flow})

        # Also look for the traditional web login flows
        elif (
            flow.request.pretty_host == "id.meijer.com"
            and flow.request.path.startswith("/oauth2/default/v1/authorize")
            and "response_type=code" in flow.request.pretty_url
            and "client_id=0oa22cbewuCICOsKz697" in flow.request.pretty_url
            and flow.response.status_code == 200
        ):
            print(f"🎯 Found traditional web login authorize event at flow {i}")
            auth_flows.append(
                {"type": "web_login_authorize", "flow_index": i, "flow": flow}
            )

        elif (
            flow.request.pretty_host == "id.meijer.com"
            and flow.request.path == "/idp/idx/identify"
            and flow.request.method == "POST"
            and flow.response.status_code == 200
        ):
            print(f"🎯 Found successful web login identify event at flow {i}")
            auth_flows.append(
                {"type": "web_login_identify", "flow_index": i, "flow": flow}
            )
...
def analyze_auth_patterns(auth_flows):
    ...
    for flow_data in auth_flows:
        ...
        if flow.response.content:
            analysis["endpoints"][endpoint]["response_sizes"].append(
...
def print_summary(analysis: Dict[str, Any], auth_flows: List[Dict[str, Any]]):
    ...
    # Group flows by type
    oauth_flows = [
        f for f in auth_flows if "oauth2" in f.get("request", {}).get("url", "")
    ]
    device_flows = [
        f for f in auth_flows if "device" in f.get("request", {}).get("url", "")
    ]
    token_flows = [
        f for f in auth_flows if "token" in f.get("request", {}).get("url", "")
    ]
