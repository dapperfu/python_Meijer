...
def test_headless_login():
    ...
    if success:
        ...
        print(f"     Code Challenge: {login.code_challenge[:20] if login.code_challenge else 'None'}...")
    ...
    if success and hasattr(login, 'device_nonce'):
        print(f"   Device Nonce: {login.device_nonce[:20] if login.device_nonce else 'None'}...")
    print()
    ...
    if success and hasattr(login, 'state_token'):
        print(f"   State Token: {login.state_token[:20] if login.state_token else 'None'}...")
    print()
    ...
    if hasattr(login, 'auth_code') and login.auth_code:
        ...
        print(f"   Result: {'✅ Success' if success else '❌ Failed'}")
        if success:
            print(f"   Access Token: {login.access_token[:20] if login.access_token else 'None'}...")
            print(f"   ID Token: {login.id_token[:20] if login.id_token else 'None'}...")
    else:
