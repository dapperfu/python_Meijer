...
def load_endpoint_data(filename: str = "endpoint_gaps_report.json") -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            "💡 Run tools/endpoint_frequency_analyzer.py first to generate the report"
        )
        return {}
...
def create_endpoint_histogram(data: Dict[str, Any], top_n: int = 20) -> None:
    ...
    fig.suptitle(
        "Meijer API: Most Frequently Hit Missing Endpoints",
        fontsize=16,
        fontweight="bold",
    )
    ...
    bars2 = ax2.barh(
        range(len(endpoint_names)), priority_scores, color=colors, alpha=0.8
    )
...
def create_implementation_roadmap(data: Dict[str, Any]) -> None:
    ...
    ax.set_title(
        "Implementation Phases and Endpoint Distribution",
        fontsize=14,
        fontweight="bold",
    )
...
def main():
    ...
    print(
        f"✅ Loaded data for {data.get('summary', {}).get('total_missing', 0)} missing endpoints"
    )
