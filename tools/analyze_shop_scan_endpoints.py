...
'''
    ...
    os.system(f"mitmdump -s {temp_script} --set flow_detail=0 --mode offline --no-ssl-insecure {log_file}")
...
def main():
    ...
    for log_name, results in all_results.items():
        if 'summary' in results:
            comprehensive_report['analysis_summary']['total_unique_endpoints'].update(results.get('all_endpoints', []))
            comprehensive_report['analysis_summary']['total_shop_endpoints'].update(results.get('shop_endpoints', {}).keys())
            comprehensive_report['analysis_summary']['total_scan_endpoints'].update(results.get('scan_endpoints', {}).keys())
            comprehensive_report['analysis_summary']['all_methods'].update(results.get('methods_found', []))
            comprehensive_report['analysis_summary']['total_status_codes'].update(results.get('status_codes_found', []))
