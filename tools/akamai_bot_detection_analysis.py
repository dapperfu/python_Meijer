...
class AkamaiBotDetectionAnalysis:
    ...
    def analyze_timing_patterns(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "average_duration": sum(r["duration"] for r in self.request_timings)
            / len(self.request_timings),
            "timing_patterns": [],
        }
        ...
        for i in range(1, len(self.request_timings)):
            interval = (
                self.request_timings[i]["start_time"]
                - self.request_timings[i - 1]["end_time"]
            )
            analysis["timing_patterns"].append(
                {
                    "step_transition": f"{self.request_timings[i - 1]['step']} -> {self.request_timings[i]['step']}",
                    "interval": interval,
                    "is_regular": abs(interval - 2.0)
                    < 0.5,  # Check if intervals are too regular
                }
            )

        # Check for suspicious patterns
        regular_intervals = sum(
            1 for p in analysis["timing_patterns"] if p["is_regular"]
        )
        analysis["suspicious_patterns"] = {
            ...
            "regularity_percentage": (
                regular_intervals / len(analysis["timing_patterns"])
            )
            * 100,
        }
    ...
    def analyze_session_context(self) -> Dict[str, Any]:
        ...
        if self.request_timings:
            ...
            analysis["request_frequency"] = (
                len(self.request_timings) / analysis["session_duration"]
            )
        ...
        if self.cookie_evolution:
            ...
            if missing_akamai:
                analysis["risk_factors"].append(
                    f"Missing Akamai cookies: {missing_akamai}"
                )
                analysis["recommendations"].append(
                    "Ensure all Akamai cookies are properly set"
                )
    ...
    def step4_web_login_identify(self) -> Dict[str, Any]:
        ...
        # Payload
        payload = {"identifier": self.username, "rememberMe": False, "stateToken": None}
        ...
        if response.status_code == 200:
            try:
                ...
                logger.info(
                    f"✅ Identify successful, state token: {self.state_token[:20] if self.state_token else 'None'}..."
                )
        ...
        try:
            ...
            analysis_results["summary"] = {
                ...
                "successful_steps": sum(
                    1
                    for step in analysis_results.values()
                    if isinstance(step, dict) and step.get("success", False)
                ),
                "failed_steps": sum(
                    1
                    for step in analysis_results.values()
                    if isinstance(step, dict) and not step.get("success", False)
                ),
                "final_status": "success"
                if analysis_results["step4"].get("success", False)
                else "failed",
                "timestamp": time.time(),
            }
            ...
            logger.info(
                f"   Successful: {analysis_results['summary']['successful_steps']}"
            )
            logger.info(f"   Failed: {analysis_results['summary']['failed_steps']}")
            logger.info(
                f"   Final Status: {analysis_results['summary']['final_status']}"
            )

            # Print bot detection insights
            self.print_bot_detection_insights(
                analysis_results["bot_detection_analysis"]
            )
    ...
    def print_bot_detection_insights(self, bot_analysis: Dict[str, Any]):
        ...
        if "suspicious_patterns" in timing:
            ...
            logger.info(
                f"   Regular intervals: {sp.get('too_regular_intervals', 0)}/{sp.get('total_transitions', 0)}"
            )
...
def main():
    ...
    try:
        ...
        if filename:
            ...
            print(
                "🔍 Bot detection patterns have been analyzed and recommendations provided"
            )
        else:
