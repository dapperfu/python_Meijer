                ...
                if results:
                    ...
                    for result in results[:5]:  # Check first 5 results
                        if hasattr(result, "upc") and result.upc:
                            ...
                            print(
                                f"    Sale Price: ${result.sale_price if result.sale_price else 'N/A'}"
                            )

                            # Check if it might be a clearance item
                            if result.sale_price and result.sale_price < result.price:
                                discount = (
                                    (result.price - result.sale_price) / result.price
                                ) * 100
        ...
        if working_products:
            ...
            for product in working_products:
                ...
                if product["sale_price"]:
                    ...
                    print(
                        f"    ${product['price']} → ${product['sale_price']} ({discount:.1f}% off)"
                    )
                else:
            ...
            print(
                f"   python scripts/test_upc_edge_cases.py {working_products[0]['upc']}"
            )
