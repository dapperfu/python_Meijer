...
class ComprehensiveAPIDiscovery:
    ...
    def _analyze_flow(self, flow: HTTPFlow, source_file: str):
        ...
        try:
            ...
            for header_name, header_value in headers.items():
                self.endpoints[endpoint_key]['headers'][header_name.lower()].add(header_value)
            
            # Store query parameters
            for param_name, param_values in query_params.items():
                self.endpoints[endpoint_key]['query_params'][param_name].update(param_values)
            ...
            if response_body and len(self.endpoints[endpoint_key]['response_bodies']) < 3:
            ...
            if 'ocp-apim-subscription-key' in headers:
                self.auth_patterns['subscription_keys'].add(headers['ocp-apim-subscription-key'])
            ...
            if flow.response:
                ...
                if response_content_type:
                    self.content_type_patterns['response_content_types'].add(response_content_type)
            
            # Track error patterns
            if status_code and status_code >= 400:
                self.error_patterns[f'{status_code}_errors'].add(f"{method} {base_path}")
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report['content_type_patterns'] = {k: list(v) for k, v in self.content_type_patterns.items()}
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.auth_patterns['bearer_tokens']:
            recommendations.append("Bearer token authentication is supported and should be prioritized")
        if self.auth_patterns['subscription_keys']:
            recommendations.append("Subscription key authentication is available as fallback")
        
        # Content type recommendations
        json_endpoints = [ep for ep, data in self.endpoints.items() 
                         if any('json' in ct.lower() for ct in data['content_types'])]
        if json_endpoints:
            recommendations.append(f"JSON endpoints found: {len(json_endpoints)} - ensure proper JSON handling")
        
        # Error handling recommendations
        error_endpoints = [ep for ep, data in self.endpoints.items() 
                          if any(status and status >= 400 for status in data['status_codes'])]
        if error_endpoints:
            recommendations.append(f"Error responses found: {len(error_endpoints)} - implement proper error handling")
        
        # Host recommendations
        if len(self.hosts) > 1:
            recommendations.append(f"Multiple API hosts detected: {list(self.hosts.keys())} - ensure proper host routing")
