...
def analyze_exact_sequence(log_file: str):
    ...
    for i, flow in enumerate(successful_sequence):
        ...
        if flow["request_body"] and flow["type"] in [
            "web_login_identify",
            "challenge_answer",
        ]:
    ...
    # 1. OAuth2 Parameters
    oauth_flow = next(
        (f for f in successful_sequence if f["type"] == "login_authorize"), None
    )
    if oauth_flow:
        url = oauth_flow["url"]
        parsed = urlparse(url)
    ...
    device_flow = next(
        (f for f in successful_sequence if f["type"] == "device_nonce"), None
    )
    if device_flow:
        print("\n📱 DEVICE FINGERPRINT:")
    ...
    # 3. Login Identify
    identify_flow = next(
        (f for f in successful_sequence if f["type"] == "web_login_identify"), None
    )
    if identify_flow:
        print("\n🔐 LOGIN IDENTIFY:")
        ...
        print(
            f"   Content-Type: {identify_flow['request_headers'].get('content-type', 'N/A')}"
        )
        print(
            f"   X-Device-Fingerprint: {identify_flow['request_headers'].get('x-device-fingerprint', 'N/A')}"
        )

    # 4. Challenge Answer
    challenge_flow = next(
        (f for f in successful_sequence if f["type"] == "challenge_answer"), None
    )
    if challenge_flow:
        print("\n🔑 CHALLENGE ANSWER:")
    ...
    # 5. Token Exchange
    token_flow = next(
        (f for f in successful_sequence if f["type"] == "token_exchange"), None
    )
    if token_flow:
        print("\n🎫 TOKEN EXCHANGE:")
        ...
        print(
            f"   X-ACF-Sensor-Data: {token_flow['request_headers'].get('x-acf-sensor-data', 'N/A')[:100]}..."
        )
    ...
    for i, flow in enumerate(successful_sequence):
        if flow["set_cookies"]:
            print(
                f"   Step {i + 1} ({flow['type']}): {len(flow['set_cookies'])} cookies set"
            )
...
def main():
    if len(sys.argv) != 2:
        ...
        print(
            "Example: python exact_request_sequence_analysis.py okta_auth_flows_raw_20250823_154001.json"
        )
        return
