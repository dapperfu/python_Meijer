...
def extract_auth_context(log_file_path: str):
    ...
    try:
        ...
        device_fp_header_positions = find_all_positions(
            content, device_fp_header_pattern
        )

        print(
            f"   Found {len(device_fp_header_positions)} device fingerprint header references"
        )
        ...
        for i, pos in enumerate(okta_ua_positions[:3]):  # Show first 3
            ...
            if context:
                ...
                okta_match = re.search(
                    r"X-Okta-User-Agent-Extended:\s*([^\r\n]+)", context
                )
                if okta_match:
