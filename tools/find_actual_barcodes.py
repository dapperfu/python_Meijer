        ...
        while True:
            ...
            try:
                ...
                if any(
                    ...
                    for keyword in ["shop", "scan", "basket", "pos", "transaction"]
                ):
...
def generate_complete_session_flow(
    results: Dict[str, Any], analysis: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    for flow in analysis["session_phases"]:
        session_flow["session_sequence"].append(
            {"phase": flow["pattern"], "position": flow["position"]}
        )

    # Add barcode scans
    for scan in analysis["scanning_sequence"]:
        session_flow["barcode_scans"].append(
            {"barcode": scan["barcode"], "position": scan["position"]}
        )
