...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python search_log_content.py <log_file> [search_term1] [search_term2] ..."
        )
        print(
            "Example: python search_log_content.py meijer_mitm.log totp feedback register"
        )
        sys.exit(1)
    ...
    search_terms = (
        sys.argv[2:]
        if len(sys.argv) > 2
        else ["totp", "feedback", "register", "signup", "create"]
    )
