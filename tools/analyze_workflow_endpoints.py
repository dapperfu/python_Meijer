    ...
    def _extract_workflow_sequences(self):
        ...
        for workflow_type, endpoints in self.workflow_endpoints.items():
            for endpoint in endpoints:
                all_endpoints.append({**endpoint, "workflow_type": workflow_type})
        ...
        for endpoint in all_endpoints:
            if not current_sequence:
                ...
                time_diff = endpoint.get("timestamp", 0) - current_sequence[-1].get(
                    "timestamp", 0
                )
    ...
    def _validate_endpoints(self):
        ...
        for workflow_type, endpoints in self.workflow_endpoints.items():
            ...
            for endpoint in endpoints:
                ...
                if any(
                    known_path in path
                    for known_path in self.known_endpoints.get(
                        workflow_type, {}
                    ).values()
                ):
        ...
        for workflow_type, endpoints in self.workflow_endpoints.items():
            for endpoint in endpoints:
                ...
                for header_name, header_value in headers.items():
                    ...
                    if (
                        header_value
                        not in header_analysis["common_headers"][header_name_lower]
                    ):
                        header_analysis["common_headers"][header_name_lower][
                            header_value
                        ] = 0
    ...
    def print_summary(self):
        ...
        if hasattr(self, "validation_results"):
            ...
            for workflow_type, validation in self.validation_results.items():
                score = validation.get("validation_score", 0.0)
                print(
                    f"  • {workflow_type}: {score:.1%} ({validation['known_endpoints']}/{validation['total_endpoints']})"
                )
