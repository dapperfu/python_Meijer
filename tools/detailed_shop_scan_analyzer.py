    ...
    for i, section in enumerate(flow_sections):
            ...
            flows["is_shop_scan_enabled"].append(
                {
                    "index": i,
                    "content": section,
                    "url": re.search(r">> ([^\s]+)", section).group(1)
        ...
        if "START_TRANSACTION" in section:
            flows["session_starts"].append(
                {"index": i, "content": section, "type": "START_TRANSACTION"}
            )

        # Look for cart update patterns
        if any(
            pattern in section for pattern in ["ADD_ITEM", "UPDATE_ITEM", "REMOVE_ITEM"]
        ):
            flows["cart_updates"].append(
                {
                    "index": i,
                    "content": section,
                    "type": re.search(r'"type":"([^"]+)"', section).group(1)
        ...
        if any(
            keyword in section.lower() for keyword in ["shop", "scan", "basket", "pos"]
        ):
            headers = {}
    ...
    for flow_type, flow_list in flows.items():
        if flow_type != "headers":  # Skip headers for sequence analysis
            for flow in flow_list:
                all_flows.append(
                    {"type": flow_type, "index": flow["index"], "data": flow}
                )
...
def generate_complete_session_template(analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    device_id = (
        list(analysis["device_id_usage"].keys())[0]
        if analysis["device_id_usage"]
