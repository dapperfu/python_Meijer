...
class AccountFlowAnalyzer:
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        if any(keyword in url.lower() for keyword in ['account', 'profile', 'user', 'receipt', 'order', 'savings']):
            flow_data = {
            ...
            if flow.request.content:
                try:
                    flow_data['request_body'] = json.loads(flow.request.content.decode('utf-8'))
                except:
            ...
            for flow_data in self.account_flows:
                if flow_data['url'] == url:
                    ...
                    if flow.response.content:
                        try:
                            flow_data['response_body'] = json.loads(flow.response.content.decode('utf-8'))
                        except:
    ...
    def done(self):
        ...
        # Print summary
        print(f"Found {len(self.account_flows)} account-related flows across {len(self.account_endpoints)} endpoints")
