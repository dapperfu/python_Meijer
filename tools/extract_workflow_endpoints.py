...
'''
    ...
    def _run_mitmdump_analysis(self, script_path: str):
        ...
        try:
            # Run mitmdump
            cmd = ["mitmdump", "-r", self.log_file_path, "-s", script_path]
            ...
            if result.returncode == 0:
                ...
                print(
                    f"❌ Mitmdump analysis failed with return code: {result.returncode}"
                )
    ...
    def _extract_header_patterns(self, data: Dict[str, Any]):
        ...
        for header_name, header_value in headers.items():
            ...
            if (
                header_name_lower
                not in self.results["header_patterns"]["common_headers"]
            ):
                self.results["header_patterns"]["common_headers"][
                    header_name_lower
                ] = {}
    ...
    def _try_alternative_analysis(self):
        ...
        self.results["analysis_note"] = (
            "Mitmdump analysis failed, alternative methods not implemented"
        )
                ...
                insights["endpoint_discoveries"].append(
                    {
                        ...
                        "count": len(events),
                        "sample_endpoints": [
                            e.get("endpoint_path", "") for e in events[:3]
                        ],
                    }
                )
