...
def analyze_aug22_flows(log_file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        cart_progression = re.findall(r'"cartNowTotal":([\d.]+).*?"cartWasTotal":([\d.]+).*?"cartSavingsTotal":([\d.]+)', content_str)
        ...
        shop_scan_content = re.findall(r'(.{300}shopandscan.{300})', content_str, re.IGNORECASE)
