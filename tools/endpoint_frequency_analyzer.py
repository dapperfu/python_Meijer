...
class EndpointFrequencyAnalyzer:
    ...
    def __init__(self):
        ...
        self.endpoint_details = defaultdict(
            lambda: {
                "count": 0,
                "methods": set(),
                "status_codes": set(),
    ...
    def generate_implementation_gaps_report(self) -> Dict[str, Any]:
        ...
        for endpoint, count in self.endpoint_counts.most_common():
            ...
            is_implemented = any(
                impl in path_lower for impl in self.implemented_endpoints
            )
        ...
        for endpoint_data in missing_endpoints[:50]:  # Top 50 most hit
            ...
            priority_endpoints.append(
                {**endpoint_data, "priority_score": priority_score}
            )
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        recommendations.append(
            f"Top 10 most frequently hit missing endpoints: {len(top_missing)}"
        )
        ...
        for category, endpoints in report["missing_endpoints_by_category"].items():
            if endpoints:
                recommendations.append(
                    f"Category '{category}': {len(endpoints)} missing endpoints"
                )

        # High priority recommendations
        high_priority = [
            ep for ep in report["implementation_priority"] if ep["priority_score"] > 50
        ]
        recommendations.append(
            f"High priority endpoints (score > 50): {len(high_priority)}"
        )

        # Implementation strategy
        recommendations.append(
            "Implementation strategy: Focus on high-frequency endpoints in core categories first"
        )
    ...
    def print_summary(self, report: Dict[str, Any]) -> None:
        ...
        for i, endpoint_data in enumerate(
            report["missing_endpoints_by_frequency"][:10], 1
        ):
            print(
                f"  {i:2d}. {endpoint_data['endpoint']} (hit {endpoint_data['count']} times)"
            )
        ...
        for i, endpoint_data in enumerate(report["implementation_priority"][:5], 1):
            print(
                f"  {i}. {endpoint_data['endpoint']} (priority: {endpoint_data['priority_score']:.1f})"
            )
