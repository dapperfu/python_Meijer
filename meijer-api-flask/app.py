    ...
    try:
        if method.upper() == "GET":
            ...
            response = requests.post(
                url, params=params, headers=request_headers, json=data, timeout=30
            )
...
def get_current_cart():
    ...
    response_data = forward_request_to_meijer(
        "digital/occ/v3/carts/current", "GET", params
    )
...
def get_homecards(card_type: str):
    ...
    # Forward to Meijer API
    response_data = forward_request_to_meijer(
        f"digital/homecards/v1/cards/{card_type}", "GET", params
    )
...
def get_mperks_balance():
    ...
    # Forward to Meijer API
    response_data = forward_request_to_meijer(
        "digital/mperks40/customer/v1/pointbalance", "GET"
    )
...
@app.route(
    "/api/meijer/digital/product-recommendations/v1/customer/<customer_id>/recommendations/<rec_type>/<store_id>/upc",
    methods=["GET"],
)
@limiter.limit("100 per minute")
def get_product_recommendations(customer_id: str, rec_type: str, store_id: str):
    ...
    params = dict(request.args)
    params.update(
        {"customer_id": customer_id, "rec_type": rec_type, "store_id": store_id}
    )

    cache_key = get_cache_key(
        f"digital/product-recommendations/v1/customer/{customer_id}/recommendations/{rec_type}/{store_id}/upc",
        params,
    )
...
@app.route(
    "/api/meijer/digital/hybris/v3/fulfillment/reservationslots",
    methods=["GET", "POST"],
)
@limiter.limit("50 per minute")
def fulfillment_reservationslots():
    ...
    # Forward to Meijer API
    response_data = forward_request_to_meijer(
        "digital/hybris/v3/fulfillment/reservationslots", method, params, data=data
    )
...
def multi_upc_lookup():
    ...
    # Forward to Meijer API
    response_data = forward_request_to_meijer(
        "digital/multi-upc/v1/upcs", method, params, data=data
    )

    # Cache successful GET responses
    if method == "GET" and "error" not in response_data:
        cache_response(
            cache_key, response_data, ttl=1800
        )  # Cache UPC lookups for 30 minutes
