...
class MeijerSecurityTester:
    ...
    def test_account_enumeration(
        self, start_id: int = 13266590, end_id: int = 13266600
    ):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if data and isinstance(data, dict) and len(data) > 0:
                            logger.warning(
                                f"⚠️  POTENTIAL VULNERABILITY: Got data for account {account_id}"
                            )
                    ...
                    endpoint_results.append({"account_id": account_id, "error": str(e)})
            ...
            for endpoint in endpoints_to_test:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if data and isinstance(data, dict) and len(data) > 0:
                            logger.warning(
                                f"⚠️  POTENTIAL VULNERABILITY: Got data with params {test_case}"
                            )
                            logger.warning(f"   Endpoint: {endpoint}")
                            logger.warning(
                                f"   Response: {json.dumps(data, indent=2)[:500]}..."
                    ...
                    results[f"{endpoint}_{test_case}"] = {
                        "status_code": response.status_code,
                        "has_data": bool(
                            data and isinstance(data, dict) and len(data) > 0
                        ),
                    }
    ...
    def test_authorization_bypass(self):
        ...
        try:
            response = self.session.get(
                f"{self.api_base}/loyalty/accounts/accounts/getAccount"
            )
    ...
    def run_all_tests(self):
        ...
        logger.info(
            "✅ Security testing completed. Results saved to security_test_results.json"
        )
        return results
