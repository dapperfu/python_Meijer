...
public final class u0 {
    ...
    private final List<v0> f8810a = new ArrayList();
    ...
    public List<v0> d(List<v0> list) {
        ...
        if (list.size() != this.f8810a.size()) {
            return null;
        }
        List<int[]> listC = c(this.f8810a.size());
        ...
        while (true) {
            ...
            int[] next = it.next();
            boolean zG = true;
            while (i10 < this.f8810a.size()) {
                if (next[i10] < list.size()) {
                    zG &= this.f8810a.get(i10).g(list.get(next[i10]));
                    if (!zG) {
                        break;
                    }
                    v0VarArr[next[i10]] = this.f8810a.get(i10);
                }
                i10++;
            }
            if (zG) {
