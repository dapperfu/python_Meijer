    ...
    public RewardClubTermsResponse fromJson(k reader) throws IllegalAccessException, NoSuchMethodException, InstantiationException, IOException, SecurityException, IllegalArgumentException, InvocationTargetException {
        ...
        while (reader.hasNext()) {
            ...
            if (iZ == -1) {
                ...
                if (lFromJson == null) {
                    throw C14410c.w("termsId", "termsId", reader);
                }
                i10 &= -2;
            } else if (iZ == 1) {
                numFromJson = this.intAdapter.fromJson(reader);
                if (numFromJson == null) {
                    throw C14410c.w("termsSourceId", "termsSourceId", reader);
                }
                i10 &= -3;
            } else if (iZ == 2) {
                lFromJson2 = this.longAdapter.fromJson(reader);
                if (lFromJson2 == null) {
                    throw C14410c.w("termsCouponId", "termsCouponId", reader);
                }
                i10 &= -5;
            } else if (iZ == 3) {
                strFromJson2 = this.stringAdapter.fromJson(reader);
                if (strFromJson2 == null) {
                    throw C14410c.w("termsText", "termsText", reader);
                }
                i10 &= -9;
            } else if (iZ == 4) {
                strFromJson = this.stringAdapter.fromJson(reader);
                if (strFromJson == null) {
                    throw C14410c.w("effectiveDate", "effectiveDate", reader);
                }
                i10 &= -17;
            } else {
                continue;
