package com.medallia.digital.mobilesdk;

import com.medallia.digital.mobilesdk.u1;
import org.json.JSONObject;

/* loaded from: classes8.dex */
class p6 implements x0 {

    /* renamed from: a, reason: collision with root package name */
    private u1.g f93546a;

    /* renamed from: b, reason: collision with root package name */
    private String f93547b;

    protected p6(JSONObject jSONObject) {
        try {
            if (jSONObject.has("value") && !jSONObject.isNull("value")) {
                this.f93547b = jSONObject.getString("value");
            }
            if (!jSONObject.has("ruleConversion") || jSONObject.isNull("ruleConversion")) {
                return;
            }
            this.f93546a = u1.g.a(jSONObject.getString("ruleConversion"));
        } catch (Exception e10) {
            a4.c(e10.getMessage());
        }
    }

    protected u1.g a() {
        return this.f93546a;
    }

    protected String b() {
        return this.f93547b;
    }

    @Override // com.medallia.digital.mobilesdk.x0
    public String toJsonString() {
        try {
            StringBuilder sb2 = new StringBuilder();
            sb2.append("{\"value\":");
            sb2.append(l3.c(this.f93547b));
            sb2.append(",\"ruleConversion\":");
            u1.g gVar = this.f93546a;
            sb2.append(l3.c(gVar != null ? gVar.toString() : null));
            sb2.append("}");
            return sb2.toString();
        } catch (Exception e10) {
            a4.c(e10.getMessage());
            return "";
        }
    }
}
