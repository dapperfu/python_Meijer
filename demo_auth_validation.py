...
from meijer import Meijer, extract_bearer_token_from_mitmproxy, load_auth_from_config_file
    ...
    with Meijer(debug=True) as meijer:
        ...
        if login_success:
            ...
            try:
                ...
                print(f"✅ Shop & Scan: {'Available' if shop_scan_enabled else 'Not available'}")
            except Exception as e:
            ...
            if not (auth_txt_available or config_available or mitmproxy_available or persistent_available):
                ...
                print("   4. Run a successful interactive login to create persistent tokens")
            else:
    ...
    if auth_txt_available:
        ...
        try:
            ...
            if bearer_auth:
                ...
                success = test_meijer.authenticate_with_bearer_token(bearer_token, user_agent)
                print(f"   {'✅' if success else '❌'} Bearer token from auth.txt: {'Success' if success else 'Failed'}")
            test_meijer.logout()
        except Exception as e:
    ...
    if mitmproxy_available:
        ...
        try:
            ...
            if result:
                ...
                test_meijer = Meijer()
                success = test_meijer.authenticate_with_bearer_token(bearer_token, user_agent)
                print(f"   {'✅' if success else '❌'} Mitmproxy bearer token: {'Success' if success else 'Failed'}")
