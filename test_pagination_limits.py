    ...
    try:
        ...
        for i, limit in enumerate(test_limits, 1):
            ...
            try:
                ...
                print(
                    f"   📈 Pagination: {len(coupons_paginated)} coupons ({pagination_time:.2f}s)"
                )
                print(
                    f"   📉 Single: {len(coupons_single)} coupons ({single_time:.2f}s)"
                )
                print(f"   {status}")

                # Show sample coupons for small limits
                if limit <= 10 and best_coupons:
                    print("   🎯 Sample coupons:")
                    for j, coupon in enumerate(
                        best_coupons[: min(3, len(best_coupons))], 1
                    ):
                        status_icon = "🟢" if coupon.is_clipped else "⚪"
                        title = (
                            coupon.title[:30] + "..."
                            if len(coupon.title) > 30
                            else coupon.title
                        )
        ...
        if failed_tests:
            ...
            for test in failed_tests:
                print(
                    f"   • Limit {test['limit']}: {test.get('error', 'Unknown error')}"
                )

        # Performance analysis
        if successful_tests:
            avg_pagination_time = sum(
                r["pagination_time"] for r in successful_tests
            ) / len(successful_tests)
            avg_single_time = sum(r["single_time"] for r in successful_tests) / len(
                successful_tests
            )
            ...
            if avg_pagination_time < avg_single_time:
                print(
                    f"   🏆 Pagination is faster by {((avg_single_time - avg_pagination_time) / avg_single_time * 100):.1f}%"
                )
            else:
                print(
                    f"   🏆 Single requests are faster by {((avg_pagination_time - avg_single_time) / avg_pagination_time * 100):.1f}%"
                )
        ...
        if len(successful_tests) == len(results):
            print("   ✅ All pagination tests passed! The system is working correctly.")
            print("   ✅ You can confidently request any number of coupons up to 1000.")
        else:
        ...
        if successful_tests:
            ...
            print(
                f"   🏆 Best performing limit: {best_performing['limit']} (got {best_performing['actual']} coupons)"
            )
