        ...
        with AuthenticatedMeijerClient(headless=True) as client:
            ...
            if success:
                ...
                if Path("auth.json").exists():
                    ...
                    print(
                        f"  - State Handle: {auth_data.get('state_handle', 'N/A')[:50]}..."
                        if auth_data.get("state_handle")
                        else "  - State Handle: N/A"
                    )
                    print(
                        f"  - Authenticator ID: {auth_data.get('authenticator_id', 'N/A')}"
                    )
                    ...
                    if "refresh_token" in auth_data:
                        print(
                            f"  - Refresh Token: {auth_data['refresh_token'][:50]}..."
                        )
                    ...
                    if new_client.is_authenticated():
                        ...
                        response = session.get(
                            "https://www.meijer.com/account", timeout=10
                        )
