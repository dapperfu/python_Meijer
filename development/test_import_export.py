...
def test_export_clipped_coupons():
    ...
    print(
        f"Created collection: {len(collection)} total, {collection.get_clipped_count()} clipped"
    )
    ...
    try:
        ...
        if exported_count == expected_count:
            print(
                f"✓ Exported {exported_count} clipped coupons (expected {expected_count})"
            )
        else:
...
def test_export_by_department():
    ...
    for i, dept in enumerate(departments):
        coupon = Coupon(
            id=f"dept{i}", name=f"{dept} Coupon {i}", department=dept, clipped=False
        )
        collection.add(coupon)
    ...
    try:
        ...
        if exported_count == expected_count:
            print(
                f"✓ Exported {exported_count} Deli coupons (expected {expected_count})"
            )
        else:
            print(f"✗ Exported {exported_count} coupons, expected {expected_count}")
            return False
...
def test_backup_and_restore():
    ...
    try:
        ...
        if len(collection) == 3 and collection.get_clipped_count() == 1:
            print(
                f"✓ Restore successful: {len(collection)} total, {collection.get_clipped_count()} clipped"
            )
        else:
            print(
                f"✗ Restore failed: {len(collection)} total, {collection.get_clipped_count()} clipped"
            )
            return False
...
def test_share_and_import():
    ...
    for i in range(2):
        coupon = Coupon(
            id=f"share{i}", name=f"Share Coupon {i}", department="Test", clipped=True
        )
        collection.add(coupon)
    ...
    try:
        ...
        imported_count = import_collection.import_shared_coupons(
            share_file, auto_clip=True
        )
        ...
        if len(import_collection) == 2 and import_collection.get_clipped_count() == 2:
            print(
                f"✓ Import successful: {len(import_collection)} total, {import_collection.get_clipped_count()} clipped"
            )
        else:
            print(
                f"✗ Import failed: {len(import_collection)} total, {import_collection.get_clipped_count()} clipped"
            )
            return False
