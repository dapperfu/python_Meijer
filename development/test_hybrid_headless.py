...
class HeadlessHybridAuth:
    ...
    def step1_selenium_bypass_akamai(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            for i, cookie in enumerate(selenium_cookies):
                logger.info(
                    f"   Cookie {i + 1}: {cookie['name']} = {cookie['value'][:30]}... (domain: {cookie['domain']})"
                )
    ...
    def step3_test_requests_access(self, session: Any, oauth_url: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    "✅ SUCCESS! Requests can access the page with Selenium cookies!"
                )
    ...
    def run_headless_hybrid_auth(self) -> bool:
        ...
        if not success:
            logger.error(
                "❌ Step 1 failed - could not bypass Akamai with headless Selenium"
            )
            return False
        ...
        if not self.step3_test_requests_access(session, oauth_url):
            logger.error(
                "❌ Step 3 failed - requests cannot access page with Selenium cookies"
            )
            return False
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print(
                "💡 This proves that headless Selenium can bypass Akamai and requests can continue the auth"
            )
            print("🚀 This approach is production-ready and CI-friendly!")
        else:
            print("❌ Headless hybrid authentication test failed")
            print(
                "💡 This suggests the issue might be deeper than just Akamai CDN blocking"
            )
