...
def demo_real_mperks_client():
    ...
    print("1. Fetch available rewards from: /digital/mperks40/customer/v1/rewardcoupons/available")
    ...
    print("• Total purchase discounts: Look for 'total', 'purchase', 'basket', 'cart', 'off'")
    ...
    print("• List of BaseReward objects (FuelReward, ProductReward, TotalPurchaseDiscountReward)")
...
def demo_reward_parsing():
    ...
    for i, reward_data in enumerate(example_rewards, 1):
        ...
        elif any(word in title_lower or word in desc_lower for word in ['total', 'purchase', 'basket', 'cart', 'off']):
            print("  Type: Total Purchase Discount")
            print("  Features: discount_amount, minimum_purchase, calculate_discount()")
        elif any(word in title_lower or word in desc_lower for word in ['free', '1 free', 'get', 'buy']):
