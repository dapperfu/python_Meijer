...
class UPCEdgeCaseDemo:
        ...
        try:
            ...
            if result:
                ...
                if result.is_weighted:
                    print(f"  Weighted Item: {result.weight_amount} {result.weight_unit}")
    ...
    def _analyze_cart_pricing(self, cart_items: List[MeijerItem], upc: str, quantity: int):
        ...
        if len(matching_items) > 1:
            ...
            if len(matching_items) == 2:
                ...
                if price_diff > 0:
                    ...
                    print(f"    🎯 Potential BOGO deal: {discount_percent:.1f}% discount on second item")
        
        # Check for clearance indicators
        clearance_items = [item for item in matching_items if item.sale_price and item.sale_price < item.price]
        if clearance_items:
            print(f"    🏷️  Clearance items detected: {len(clearance_items)} item(s) on sale")
            for item in clearance_items:
                discount = ((item.price - item.sale_price) / item.price) * 100
                print(f"      {item.title}: {discount:.1f}% off (${item.price} → ${item.sale_price})")
        ...
        for quantity in test_quantities:
            ...
            if success:
                ...
                if matching_items:
                    ...
                    print(f"  Qty {quantity}: Total=${total_price:.2f}, Avg=${avg_price:.2f}")
                else:
    ...
    def _analyze_pricing_patterns(self, pricing_data: Dict[int, Dict]):
        ...
        if price_breaks:
            ...
            for break_info in price_breaks:
                ...
                print(f"      At quantity {threshold}: ${old_price:.2f} → ${new_price:.2f} (save ${savings:.2f} each)")
        ...
        if len(quantities) >= 5:
            small_qty_avg = sum(pricing_data[q]['unit_price'] for q in quantities[:3]) / 3
            large_qty_avg = sum(pricing_data[q]['unit_price'] for q in quantities[-3:]) / 3
            
            if large_qty_avg < small_qty_avg:
                discount = ((small_qty_avg - large_qty_avg) / small_qty_avg) * 100
                print(f"    📦 Bulk discount detected: {discount:.1f}% off for larger quantities")
