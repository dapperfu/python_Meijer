...
class ComplexCartDemo:
    ...
    async def demo_cart_validation(self):
        ...
        try:
            ...
            if validation_issues:
                ...
                critical_issues = [issue for issue in validation_issues if issue.severity == CartValidationStatus.CRITICAL]
                error_issues = [issue for issue in validation_issues if issue.severity == CartValidationStatus.ERROR]
                warning_issues = [issue for issue in validation_issues if issue.severity == CartValidationStatus.WARNING]
    ...
    async def demo_cart_optimization(self):
        ...
        try:
            ...
            optimization_strategies = ["price", "availability", "substitutions", "bundling"]
            ...
            if optimization_result:
                ...
                print(f"   Savings percentage: {optimization_result.savings_percentage:.1f}%")
            ...
            if split_result:
                ...
                if split_result.split_carts:
                    ...
                    for i, split_cart in enumerate(split_result.split_carts[:3]):
                        ...
                        print(f"      Category: {split_cart.get('category', 'Unknown')}")
                        print(f"      Delivery: {split_cart.get('deliveryMethod', 'Unknown')}")
            else:
        ...
        try:
            ...
            if analytics_result:
                ...
                if "categoryBreakdown" in analytics_result:
                    print(f"   Categories: {len(analytics_result['categoryBreakdown'])}")
                if "averageItemPrice" in analytics_result:
                    print(f"   Average item price: ${analytics_result['averageItemPrice']:.2f}")
                if "estimatedSavings" in analytics_result:
                    print(f"   Estimated savings: ${analytics_result['estimatedSavings']:.2f}")
            else:
        ...
        try:
            ...
            if edge_case_result:
                ...
                if "validationIssues" in edge_case_result:
                    print(f"   Validation issues found: {len(edge_case_result['validationIssues'])}")
                if "autoResolvedIssues" in edge_case_result:
                    print(f"   Auto-resolved issues: {len(edge_case_result['autoResolvedIssues'])}")
                if "manualResolutionRequired" in edge_case_result:
                    print(f"   Manual resolution required: {len(edge_case_result['manualResolutionRequired'])}")
                if "optimizationResult" in edge_case_result:
                    print(f"   Optimization result: {edge_case_result['optimizationResult']}")
                ...
                if edge_case_result['cartStatus'] == 'healthy':
                    ...
                    if edge_case_result.get('manualResolutionRequired'):
                        ...
                        print("   • Consider running cart optimization after resolution")
                else:
...
async def main():
    ...
    print("This demo showcases the advanced cart operations and edge case handling system.")
