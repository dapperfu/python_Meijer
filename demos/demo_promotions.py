...
class PromotionsDemo:
    ...
    async def demo_basic_promotions(self):
        ...
        try:
            ...
            if customer_promotions:
                print(f"✅ Found {len(customer_promotions)} customer-eligible promotions")
            else:
    ...
    async def demo_promotion_creation(self):
        ...
        try:
            ...
            print(f"   Duration: {sample_promotion.start_date.strftime('%Y-%m-%d')} to {sample_promotion.end_date.strftime('%Y-%m-%d')}")
        ...
        try:
            ...
            if flash_sales:
                ...
                for sale in flash_sales[:3]:  # Show first 3
                    ...
                    print(f"     Duration: {sale.start_time.strftime('%H:%M')} - {sale.end_time.strftime('%H:%M')}")
            ...
            print(f"   Duration: {sample_flash_sale.start_time.strftime('%H:%M')} - {sample_flash_sale.end_time.strftime('%H:%M')}")
    ...
    async def demo_deal_optimization(self):
        ...
        try:
            ...
            cart_total = sum(item["price"] * item["quantity"] for item in sample_cart_items)
            ...
            if optimizations:
                ...
                for opt in optimizations:
                    ...
                    print(f"     Savings: ${opt.savings_amount:.2f} ({opt.savings_percentage:.1f}%)")
            ...
            if stackable_discounts:
                ...
                for discount in stackable_discounts[:3]:
                    print(f"   • {discount.get('name', 'Unknown')}: ${discount.get('amount', 0):.2f}")
            else:
    ...
    async def demo_maximum_savings_calculation(self):
        ...
        try:
            ...
            if savings_result:
                ...
                print(f"   Savings percentage: {savings_result['savingsPercentage']:.1f}%")
                print(f"   Optimization score: {savings_result['optimizationScore']:.2f}")
                
                if savings_result['appliedPromotions']:
                    print(f"   Applied promotions: {len(savings_result['appliedPromotions'])}")
