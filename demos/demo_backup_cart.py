    ...
    try:
        ...
        if cart.has_items:
            ...
            for item in cart.items:
                backup_indicator = (
                    f" [Has {item.backup_item_count} backups]"
                    if item.has_backup
                    else ""
                )
                print(
                    f"  • {item.product_name or f'UPC {item.product_code}'}{backup_indicator}"
                )
                print(f"    Entry Number: {item.entry_number}")
                print(f"    Quantity: {item.current_quantity}")
...
def demo_cart_backup_management():
    ...
    try:
        ...
        if cart.has_items:
            ...
            success = cart.remove_backup_item_from_cart_item(
                item1_entry, "111111111111"
            )
            print(f"  Removed backup: {'✅' if success else '❌'}")
            print(
                f"  Item 1 backups after removal: {cart.get_cart_item_backups(item1_entry)}"
            )
