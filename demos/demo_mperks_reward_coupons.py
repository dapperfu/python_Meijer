...
def demo_reward_coupons():
    ...
    try:
        ...
        if available_coupons:
            ...
            fuel_coupons = [
                c for c in available_coupons if c.reward_coupon_type == "Fuel"
            ]
            if fuel_coupons:
                print("\n   🚗 Fuel Rewards:")
                for coupon in fuel_coupons[:3]:  # Show first 3
                    ...
                    if coupon.days_until_expiry is not None:
                        if coupon.days_until_expiry > 0:
                            ...
                            print(
                                f"        Expired {abs(coupon.days_until_expiry)} days ago"
                            )
        ...
        if fuel_coupon:
            ...
            if point_balance.total_points >= fuel_coupon.point_cost:
                print(
                    f"   ✅ Sufficient points available ({point_balance.total_points:,} >= {fuel_coupon.point_cost:,})"
                )

                # 4. Demonstrate purchasing the coupon (commented out for safety)
                print("\n💳 Purchase simulation (commented out for safety):")
                print(
                    "   To actually purchase this coupon, uncomment the following code:"
                )
                ...
                print(
                    f"   ❌ Insufficient points ({point_balance.total_points:,} < {fuel_coupon.point_cost:,})"
                )
        else:
    ...
    try:
        ...
        if fuel_coupon:
            ...
            if fuel_coupon.coupon_id == 6521640:
                ...
                print(
                    f"   ⚠️  Coupon ID {fuel_coupon.coupon_id} doesn't match expected 6521640"
                )

            # Show purchase simulation
            print(f"\n💳 Purchase Simulation for Coupon {fuel_coupon.coupon_id}:")
            print(
                "   POST /digital/mperks40/customer/v1/rewardcoupons/available/6521640/buy"
            )
