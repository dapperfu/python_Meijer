...
def test_pure_requests_step_by_step():
    ...
    try:
        ...
        if client._step1_oauth_authorize():
            ...
            print(
                f"   State token: {client.state_token[:50]}..."
                if client.state_token
                else "   No state token"
            )
        else:
            print("❌ Step 1 failed")
        ...
        if client._step2_device_fingerprint():
            ...
            print(
                f"   Device nonce: {client.device_nonce[:50]}..."
                if client.device_nonce
                else "   No device nonce"
            )
        else:
            print("❌ Step 2 failed")
            return False
        ...
        if client._step4_web_login_identify(username):
            ...
            print(
                f"   State handle: {client.state_handle[:50]}..."
                if client.state_handle
                else "   No state handle"
            )
        else:
            print("❌ Step 4 failed")
            return False
        ...
        if success:
            ...
            if Path("auth.json").exists():
                ...
                try:
                    ...
                    print(
                        f"   Access token: {auth_data.get('access_token', 'N/A')[:50]}..."
                    )
...
def analyze_session_details():
    ...
    try:
        ...
        print(
            f"   ID Token: {auth_data.get('id_token', 'N/A')[:50] if auth_data.get('id_token') else 'N/A'}..."
        )
