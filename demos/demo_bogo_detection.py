...
def demo_bogo_detection():
    ...
    # For demo purposes, we'll show the structure
        ...
        bogo_results_structure = {
            ...
            "bogo_type": "BOGO40",
            "price_progression": [
                {"quantity": 1, "total_price": 10.99, "unit_price": 10.99, "price_per_item": 10.99},
                {"quantity": 2, "total_price": 17.58, "unit_price": 8.79, "price_per_item": 8.79},
                {"quantity": 10, "total_price": 87.92, "unit_price": 8.79, "price_per_item": 8.79}
            ],
            "optimal_quantity": 2,
        ...
        for key, value in bogo_results_structure.items():
            if key == "price_progression":
                ...
                for i, price_info in enumerate(value):
                    print(f"    {i+1}. Qty {price_info['quantity']}: ${price_info['total_price']:.2f} total, ${price_info['price_per_item']:.2f} each")
            else:
                print(f"  {key}: {value}")
