...
class OAuthConfig:
    ...
    redirect_uri: str = (
        "https://localhost:45678/callback"  # Web-compatible redirect URI on high port
    )
    mobile_redirect_uri: str = (
        "com.meijer.mobile.meijer:/login"  # Original mobile app URI
    )
...
class MeijerComprehensiveClient:
    ...
    def get_authorization_url(
        self, use_mobile_uri: bool = False
    ) -> tuple[str, str, str]:
        """
        ...
        redirect_uri = (
            self.oauth_config.mobile_redirect_uri
            if use_mobile_uri
            else self.oauth_config.redirect_uri
        )
    ...
    def authenticate_interactive(self) -> bool:
        ...
        try:
            ...
            print(
                f"   com.meijer.mobile.meijer:/login?code=AUTHORIZATION_CODE&state={state}"
            )
            ...
            if self.auth_tokens and self.auth_tokens.id_token:
                ...
                if len(parts) == 3:
                    ...
                    self.user_info = UserInfo(
                        ...
                        updated_at=datetime.fromtimestamp(payload.get("updated_at", 0))
        ...
        try:
            ...
            for offer_data in offers_data.get("offers", []):
                offer = MeijerOffer(
                    ...
                    valid_from=datetime.fromisoformat(offer_data["validFrom"])
                    if offer_data.get("validFrom")
                    else None,
                    valid_until=datetime.fromisoformat(offer_data["validUntil"])
                    if offer_data.get("validUntil")
                    else None,
            ...
            info.update(
                {
                    ...
                    "expires_at": self.auth_tokens.expires_at.isoformat()
                    if self.auth_tokens.expires_at
                    else None,
                    "time_until_expiry": str(self.auth_tokens.time_until_expiry())
                    if self.auth_tokens.time_until_expiry()
                    else None,
                }
            )
    ...
    def is_authenticated(self) -> bool:
        ...
        return (
            self.auth_status == AuthenticationStatus.AUTHENTICATED
            and self.auth_tokens
            and not self.auth_tokens.is_expired()
        )
    ...
    def authenticate_with_oauth_tokens(
        self,
        access_token: str,
    ...
    def login_with_oauth_tokens(
        ...
        try:
            ...
            if access_token is None:
                access_token, refresh_token, user_agent, expires_in, scope = (
                    read_oauth_auth_file()
                )
    ...
    def _check_and_refresh_tokens(self) -> bool:
        ...
        if self.auth_tokens.is_expired(buffer_seconds=300):
            if self.auth_tokens.refresh_token:
                ...
                self.logger.warning(
                    f"⚠️  Token expires soon but no refresh token available"
                )
                return False
    ...
    def make_authenticated_request(
        ...
        if (
            ...
            and self.auth_tokens
            and self.auth_tokens.refresh_token
        ):
    ...
    def login_with_bearer_token(
        self, bearer_auth_file: str = "bearer_auth.txt"
    ) -> bool:
        """
    ...
    def authenticate_with_requests(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            csrf_token = None
            csrf_input = soup.find("input", {"name": "_csrf"}) or soup.find(
                "input", {"name": "csrf"}
            )
            if csrf_input:
            ...
            if form_method == "POST":
                response = session.post(
                    submit_url, data=login_data, allow_redirects=False
                )
            else:
                response = session.get(
                    submit_url, params=login_data, allow_redirects=False
                )
            ...
            if auth_code:
                ...
                if success:
                    self.logger.info(
                        "✅ Requests-based authentication completed successfully!"
                    )
                    return True
                else:
...
def read_oauth_auth_file(
    filepath: str = "auth.txt",
) -> Tuple[str, Optional[str], str, Optional[int], Optional[str]]:
    """
