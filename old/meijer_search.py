...
class MeijerSearchResults:
    ...
    def __str__(self) -> str:
        ...
        return (
            f"MeijerSearchResults(query='{self.query}', "
...
class MeijerSearch:
    ...
    def search(
        ...
        try:
            ...
            if isinstance(search_data, dict):
                ...
                elif "response" in search_data and isinstance(
                    search_data["response"], dict
                ):
            ...
            logger.info(
                f"Search for '{query}' page {page}: {len(items)} items, "
    ...
    def autocomplete(self, partial_query: str, num_suggestions: int = 8) -> List[str]:
        """
    ...
    def browse_category(
        ...
        try:
            ...
            if isinstance(browse_data, dict):
                ...
                elif "response" in browse_data and isinstance(
                    browse_data["response"], dict
                ):
            ...
            logger.info(
                f"Browse category '{collection_id}' page {page}: {len(items)} items, "
                f"total ~{total_results}"
            )
            return results
            ...
            logger.error(
                f"Browse failed for category '{collection_id}' page {page}: {e}"
            )
            # Return empty results container
            return MeijerSearchResults(
                items=[],
